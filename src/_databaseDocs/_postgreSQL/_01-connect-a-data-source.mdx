...
The CLI has provided an `.env` file for our connector. We can add a key-value pair of `CONNECTION_URI` along with the
connection string itself to this file and our connector will use this to connect to our PostgreSQL database. The file,
after adding the `CONNECTION_URI` should look like this example:
