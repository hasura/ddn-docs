...
```bash
ddn connector init my_pg  --subgraph <subgraph-name>  --hub-connector hasura/postgres
```
...
In the `<subgraph-name>/connector/my_pg` directory which we specified in the command, the CLI created:
...
Typically, connectors default to port `8080`. Each time you add a conenctor, we recommend incrementing the published
port by one to avoid port collisions. For example:

Change the value of your `my_pg` connetor's published port in your
`<subgraph-name>/connector/my_pg/docker-compose.my_pg.yaml` to `8082` to avoid port collisions with other connectors:
...
string you provide in your subgraph's `.env.<subgraph-name>` file.

:::

## Step 3. Include the PostgreSQL connector's compose file

In our main `docker-compose.hasura.yaml` in the root of the project, add the following at the top taking care to replace
...
The CLI has provided an `.env.local` file for our connector in the `<subgraph-name>/connector/my_pg` directory. We can
...
OTEL_SERVICE_NAME=<subgraph-name>_my_pg
#highlight-start
...
```bash
ddn connector introspect --connector <subgraph-name>/connector/my_pg/connector.yaml
```
