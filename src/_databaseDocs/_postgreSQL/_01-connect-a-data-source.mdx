...
```bash
ddn connector init mypg  --subgraph <subgraph-name>  --hub-connector hasura/postgres
```
...
We're naming the connector `mypg` in this example, but you can call it whatever makes sense to you. For example, if this
connector is integrating a database all about product metadata, it would make sense to name it something like
...
be saved in the subgraph directory. In this example the CLI will create a `connector/mypg` directory if it doesn't
exist.
...
The name of the connector and the directory in which the configuration is stored, `mypg` in this example, should match
...
We're specifying that this connector should be the: `hasura/mypg`, connector listed in the
[Connector Hub](https://hasura.io/connectors/postgres).

### What did this do?

In the `<subgraph-name>/connector/mypg` directory which we specified in the command, the CLI created:
...
- A `docker-compose.mypg.yaml` a file to run the MongoDB data connector locally in Docker.
...
Change the value of your `mypg` connetor's published port in your
`<subgraph-name>/connector/mypg/docker-compose.mypg.yaml` to `8082` to avoid port collisions with other connectors
...
The CLI has provided an `.env.local` file for our connector in the `<subgraph-name>/connector/mypg` directory. We can
...
OTEL_SERVICE_NAME=<subgraph-name>_mypg
#highlight-start
...
connect to it directly from the data connector. However, if you deploy your connector to Hasura DDN, the cloud-hotsed
...
```bash
ddn connector introspect --connector <subgraph-name>/connector/mypg/connector.yaml
```
...
<!-- prettier-ignore -->
With this JSON configuration, which represents the tables in PostgreSQL, we can now use it to <BaseUrlLink to="/getting-started/connect-to-data/link-a-connector" text="generate Hasura metadata" />.
