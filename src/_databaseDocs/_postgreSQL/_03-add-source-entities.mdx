...
If you look in your `my_subgraph/metadata/my_pg.hml` file, you'll see `collections` for each table in
...
ddn model add --connector-link my_pg --name users
```

:::tip Casing matters

When adding a model, it is critical to use the same casing as the `collection` from your connector's configuration
file.
...
When you ran this command, the CLI generated an `hml` file in the `models` directory for the `collection` you passed
...
ddn connector-link update <connector-name> --subgraph my_subgraph --add-all-resources
```
