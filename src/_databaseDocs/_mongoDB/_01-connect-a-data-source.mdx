...
be saved in the subgraph directory. In this example the CLI will create a `my_subgraph/connector/my_mongo` directory
if it doesn't exist.
...
`my_subgraph/connector/my_mongo/docker-compose.my_mongo.yaml` to `8082` to avoid port collisions with other
connectors:
