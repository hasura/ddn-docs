...
You're now ready to connect to your MongoDB database via the
[MongoDB data connector](https://hasura.io/connectors/mongodb).
...
First, we're naming the connector `my_mongo` in this example, but you can call it whatever makes sense to you. For
...
Second, We want to keep our project organized with each subgraph's data connectors configuration located in the relevant
subgraph's directory. To do this we're passing the required `--dir` flag to tell the CLI that the configuration files
it's generating should be saved in the subgraph directory and then a `connector/mymongo` directory which the CLI should
...
If you already have one you can connect to, you can go ahead and do that. Hasura DDN will not modify your database in
any way, so you can use an existing database without any worries.

:::info Environment-specific caveats

**Local Mongo**

If you're using a local MongoDB database — such as through [Docker](https://hub.docker.com/_/mongodb) — you'll need to
...
The CLI automatically scaffolds out an `.env` file for our connector. We can add a key-value pair of
`MONGODB_DATABASE_URI` along with the connection string itself to this file and our connector will use this to connect
to our MongoDB database. The file, after adding the `MONGODB_DATABASE_URI` should look like this example:
