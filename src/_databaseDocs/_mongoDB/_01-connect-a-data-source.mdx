...
`<subgraph-name>/connector/my_mongo/docker-compose.my_mongo.yaml` to `8082` to avoid port collisions with other
connectors:
...
In our main `docker-compose.hasura.yaml` in the **_root of the project_**, add the following at the top taking care to
replace `<subgraph-name>` with your subgraph:
...
<BaseUrlLink to="/getting-started/connect-to-data/link-a-connector" text="generate Hasura metadata" />.
