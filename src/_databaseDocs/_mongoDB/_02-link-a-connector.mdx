...
MongoDB connector. Because we've already started up these services in Docker, we'll add these to the `subgraph_name/.
env.subgraph_name` file.

```env
<subgraph-name>_<connector-name>_READ_URL=http://local.hasura.dev:8082
<subgraph-name>_<connector-name>_WRITE_URL=http://local.hasura.dev:8082
```
...
After this command runs, you can open your `<subgraph>/metadata/my_mongo/my_mongo.hml` file and see your metadata
