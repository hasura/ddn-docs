...
We want to connect our ClickHouse instance to our API. To do this, we use the Hasura ClickHouse data connector to
...
To initialize the ClickHouse connector, run the following.

```bash title="From the root of your project, run:"
ddn connector init my_clickhouse --subgraph my_subgraph --hub-connector hasura/clickhouse
```
...
example, if this connector is integrating a database all about user metadata, it would make sense to name it something
like `user_metadata_clickhouse_connector`.
...
- A `docker-compose.my_clickhouse.yaml` a file to run the ClickHouse data connector locally in Docker.
...
Change the value of your `my_clickhouse` connector's published port in your
`my_subgraph/connector/my_clickhouse/docker-compose.my_clickhouse.yaml` to `8082` to avoid port collisions with other
connectors:

```yaml title="my_subgraph/connector/my_clickhouse/docker-compose.my_clickhouse.yaml"
ports:
  - mode: ingress
    target: 8080
    #highlight-start
...
#highlight-start
CONNECTION_URI=<clickhouse-connection-uri>
...
If you're using a local ClickHouse database — such as through [Docker](https://hub.docker.com/_/clickhouse) — you can
...
[ngrok](https://ngrok.com/) to tunnel your database's connection. This will expose the port, most likely `9000`, on
...
```bash title="From the root of your project run:"
ddn connector introspect --connector my_subgraph/connector/my_clickhouse/connector.yaml
```

If you look at the `configuration.json` for your connector, you'll see metadata describing your ClickHouse schema.
...
<!-- prettier-ignore -->
With this JSON configuration, which represents the tables in ClickHouse, we can now use it to <BaseUrlLink to="/getting-started/connect-to-data/create-source-metadata" text="generate Hasura metadata" />.
