...
We'll use the `dotenv` and `@clickhouse/client` packages to access our ClickHouse credentials and connection string,
saved as an environment variables, and to connect to our database.
...
export async function insertUser(userData: { name: string; email: string }): Promise<string> {
  ...
  await client.exec({
    query: insertQuery,
    query_params: [userData.name, userData.email],
  });
