...
const HasuraConBanner = (props) => {
  const isSnowFlakeSection = props.location.pathname.startsWith(
    `/docs/latest/databases/snowflake`
  );

  const isObservabilitySection = props.location.pathname.startsWith(
    `/docs/latest/observability`
  );

  const isSecuritySection = props.location.pathname.startsWith(
    `/docs/latest/security`
  );

  const isMySQLSection = props.location.pathname.startsWith(
    `/docs/latest/databases/mysql`
  );

  const isOracleSection = props.location.pathname.startsWith(
    `/docs/latest/databases/oracle`
  );

  const isMariaDBSection = props.location.pathname.startsWith(
    `/docs/latest/databases/mariadb`
  );
    ...
    return (
      <div className={styles["product-launch-webinar-bg"]}>
        <a
          ...
          href="https://hasura.io/events/webinar/product-launch/"
        >
          ...
          <div className={styles["content-div"]}>
            ...
            <div
              className={
                styles["hasura-con-register"] +
                " " +
                styles["hasura-con-register-mobile-hide"]
              }
            >
              View Recording
              <ArrowRight />
  ...
  if (isSnowFlakeSection) {
    return (
      <div className={styles["snowflake-bg"]}>
        <a
          className={styles["webinar-banner"]}
          href="https://hasura.io/events/webinar/snowflake-and-postgresql/"
        >
          <div className={styles["hasura-con-brand"]}>
            <img
              className={styles["brand-light"]}
          ...
          <div className={styles["content-div"]}>
            <h3>
              Combining Snowflake and PostgreSQL to build low-latency apps on
  ...
  if (isSecuritySection) {
    return (
      <div className={styles["security-bg"]}>
        <a
          className={styles["webinar-banner"]}
          href="https://hasura.io/events/webinar/securing-your-api-with-hasura/"
        >
          <div className={styles["hasura-con-brand"]}>
            <img
              className={styles["brand-light"]}
  ...
  return (
    <a
      className={styles["hasura-con-banner"]}
    ...
    >
      ...
      <div className={styles["hasura-con-space-between"]}>
        <div>
          <div className={styles["hasura-con-23-title"]}>
            The fourth annual Hasura User Conference
          </div>
        </div>
        <div
          className={
            styles["hasura-con-register-button"] +
            " " +
            styles["hasura-con-register-mobile-hide"]
          }
        >
