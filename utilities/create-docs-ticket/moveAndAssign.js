...
const moveTicketToSprint = async (sprintId, ticketId) => {
  const sprintResponse = await fetch(`${process.env.DOCS_JIRA_BASE_URL}/rest/agile/1.0/sprint/${sprintId}/issue`, {
    method: 'POST',
    headers: config.headers,
  ...
  if (sprintResponse.ok) {
    ...
    console.error('Error moving issue to the sprint:', sprintResponse.statusText);
  }
};
...
const assignReviewer = async ticketId => {
  ...
  const assignResponse = await fetch(`${process.env.DOCS_JIRA_BASE_URL}/rest/api/2/issue/${ticketId}/`, {
    method: 'PUT',
    headers: config.headers,
    body: JSON.stringify({
      fields: {
  ...
  if (assignResponse.ok) {
    ...
    console.log(`${assigneeDetails.reviewer} has been assigned to review this ticket in Jira.`);
  } else {
