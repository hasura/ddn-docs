...
| `tokenLocation` | Object                              | true     | Declares the request header or the cookie (with cookie name) from which to read the JWT. Default: ` { "type": `[Token location value](#jwt-token-location-value) }.                        |
