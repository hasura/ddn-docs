---
description: Learn how to authenticate requests using JWT with Hasura. Understand how to configure Hasura Engine to use JWT mode, extract session variable claim values, and enable JWT mode.
keywords:
  - hasura
  - jwt
  - graphql
...
[//]: # (:::info Prerequisite for JWT mode TODO)
...
```yaml
mode:
  # either `viaKey` or `viaJWK` is expected.
  viaKey:
    type: optional-type-of-key
    key: required-key-as-string
  viaJWK: required-JWK-url
claimsConfig:
  # either `namespace` or `claimsMap`. If nothing is provided, default to `namespace` with its default values.
  namespace:
    path: optional-json-pointer-to-the-claims-default-https://hasura.io/jwt/claims
    format: optional-claims-format-default-Json
  claimsMap:
...
header: "<optional-object-to-indicate-cookie-or-authorization-header-or-custom->"
...
|-----------------------|-------------------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `mode`                | [`JWT mode`](#jwt-mode) | true     | JWT mode to configure the JWT authentication.                                                                                                                                                  |
| `claimsNamespacePath` | JSON Pointer            | false    | Default path where the Hasura claims object should be looked up in the decoded JWT. Default: (Default: "https:~1~1hasura.io~1jwt~1claims" (JSON Pointer for `https://hasura.io/jwt/claims`)).    |
| `claimsFormat`        | String                  | false    | Format of the Hasura claims in the decoded JWT. Possible values: `json`, `stringifiedJson`. Default: `json`.                                                                                        |
| `claimsMap`           | Object                  | false    | Object of session variables to the pointer or literal value where the Hasura claims object should be looked up in the decoded JWT.                                                                 |
...
| `header`              | Object                  | false    | Declares the request header or the cookie (with cookie name) from which to read the JWT. Default: ```json { "type": "Authorization" }```.                                                       |
...
| Field    | Type                | Required | Description                                                                                                  |
|----------|---------------------|----------|--------------------------------------------------------------------------------------------------------------|
| `viaKey` | [`ViaKey`](#viakey) | false    | Static JWT mode, when the `type` and the `key` fields are known and aren't expected to change during runtime. |
| `viaJWK` | URL                 | false    | JWK URL from which the graphql-engine will determine the `type` and `key` fields from dynamically.            |

:::info Note

Either, the `viaKey` or the `viaJWK` is expected.
:::
...
| Field  | Type   | Required | Description                                                                                                                                                            |
|--------|--------|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `type` | String | false    | Algorithm used to decode the JWT. Possible values are: `HS256`, `HS384`, `HS512`, `ES256`, `ES384`, `RS256`, `RS384`, `RS512`, `PS256`, `PS384`, `PS512` and `EdDSA`. |
| `key`  | String | false    | Key to decode the JWT.                                                                                                                                                  |


#### JWT claims config {#jwt-claims-config}

| Field       | Type                                 | Required | Description                                                                                                                                                                            |
|-------------|--------------------------------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `namespace` | [`Namespace`](#jwt-claims-namespace) | false    | Used when all of the Hasura claims are present in a single object within the decoded JWT.                                                                                               |
| `claimsMap` | [`Claims Map`](#jwt-claimsmap)       | false    | Can be used when Hasura claims are not all present in the single object, but individual claims are either provided a default value or a JSON pointer within the decoded JWT. |
...
##### JWT claims namespace {#jwt-claims-namespace}
...
##### JWT claims map {#jwt-claimsmap}

| Field                    | Type                                                     | Required | Description                              |
|--------------------------|----------------------------------------------------------|----------|------------------------------------------|
| `x-hasura-allowed-roles` | [`Claims Map Object ([String])`](#jwt-claims-map-object) | true     | JSON pointer to the Hasura claims object. |
| `x-hasura-default-role`  | [`Claims Map Object (String)`](#jwt-claims-map-object)   | true     | JSON pointer to the Hasura claims object. |
| Custom claim*            | [`Claims Map Object (String)`](#jwt-claims-map-object)   | false    | JSON pointer to the custom Hasura claim. |
...
###### JWT claims map object `<T>` {#jwt-claims-map-object}

| Field     | Type                                                       | Required | Description                                                                                                                             |
|-----------|------------------------------------------------------------|----------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `literal` | `<T>`                                                      | false    | Literal value of the Hasura claim.                                                                                                       |
| `path`    | [JSON claims map path object](#jwt-claims-map-path-object) | false    | Get claim value using a JSON pointer to the Hasura claim with a fallback default value, if the value is not present at the JSON pointer. |

###### JWT claims map object `<T>` {#jwt-claims-map-object}

| Field     | Type                                                       | Required | Description                                                                   |
|-----------|------------------------------------------------------------|----------|-------------------------------------------------------------------------------|
| `path`    | [JSON claims map path object](#jwt-claims-map-path-object) | false    | JSON pointer to the Hasura claim.                                              |
...
#### x-hasura-allowed-roles {#x-hasura-allowed-roles}
...
#### x-hasura-default-role {#x-hasura-default-role}
...
- Hasura GraphQL Engine only has access to JWT claims in the `https://hasura.io/jwt/claims` or
  [custom defined](#claims-namespace-path) namespace.
- All `x-hasura-*` values should be of type `String`, they will be converted to the right type automatically.
- In JWT mode on a secured endpoint JWT authentication is **enforced**
...
### claimsNamespacePath {#claims-namespace-path}
...
##### Configure `claimsNamespacePath` {#configure-claims-namespace-path}
