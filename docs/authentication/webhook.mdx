...
- You can configure Hasura to run in webhook mode by running the GraphQL Engine by adding a [auth config](../auth/authentication.mdx#webhook-auth-config)
  metadata object
  endpoint.
...
`POST` requests will receive **all the client headers**. Given a request
like:
