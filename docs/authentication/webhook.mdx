...
- You can configure Hasura to run in webhook mode by running the GraphQL Engine by adding an
  [auth config](./auth-config.mdx#webhook-auth-config) metadata object endpoint.
