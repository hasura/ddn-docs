...
## Logging
You can make the CLI print logs to see what is happening, and it can be useful in debugging issues if you wish to do so. You can enable logs using `--log-level` flag in all commands (or, using the env var `HASURA_CLI_LOG_LEVEL`), and it supports the ususal logging levels (error, warn, info, debug, trace). There are filters installed on logs, and by default the CLI only prints logs from Hasura crates that it uses. To print logs from all crates, append `-all` to the log level you want (like, `--log-level debug-all`).
### Examples
