...
1. Start a Postgres Database (you can use this [docker image](https://hub.docker.com/_/postgres)).
2. Start the Secure Connect Tunnel daemon using the following command:

```bash
hasura3 local tunnel daemon start
```

3. Create a new Tunnel using the command:

```bash
hasura3 cloud tunnel create <POSTGRES_URL>
```

The Tunnel URL will be returned from this command and you'll use this URL in your Metadata. Update the
`postgres_database_url` in your Metadata file with the Tunnel URL and apply the `metadata.yaml` to read:

```yaml
postgres_database_url: 'postgres://<TUNNEL_URL>'
```

:::info What's a Tunnel?

A **Secure Connect Tunnel** (Tunnel, for short) is a new feature designed to allow you to connect your local databases
to Hasura Cloud.

**Do note: if you haven't been following along with the CLI instructions previous to this step, you'll need to
authenticate to Hasura using a [PAT](#step-1-log-into-hasura).**
...
Models aren't imported when authoring Metadata locally. You'll need to...
...
It maybe helpful to think of builds as git commits. Since each is deployed on Hasura Cloud, it can be shared with other
users.
...
production endpoint. To find this value, run the following:
