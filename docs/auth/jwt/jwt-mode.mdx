...
JWT mode requires that the client making the query sends a valid JWT to the Hasura Engine endpoint. This JWT is provided
...
Below, we're demonstrating using the `BearerAuthorization` format with a fixed secret key from an environment variable.
However, Hasura DDN supports other methods for both
...
## Step 2. Define the JWT with claims

Your auth service should include an object with a key of `claims.jwt.hasura.io` in the JWT's claims. Within this, each
claim should be prefixed with `x-hasura-*` and include the relevant information.
...
[here](https://jwt.io/#debugger-io?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE1MTYyMzkwMjIsImV4cCI6MTUxNjMyNTQyMiwiY2xhaW1zLmp3dC5oYXN1cmEuaW8iOnsieC1oYXN1cmEtZGVmYXVsdC1yb2xlIjoiYWRtaW4iLCJ4LWhhc3VyYS1hbGxvd2VkLXJvbGVzIjpbInVzZXIiLCJhZG1pbiJdLCJ4LWhhc3VyYS11c2VyLWlkIjoxMjM0fX0.84qumMkdpgmvU_pJBugScWxGRmy-AF2KDiCYd7G0Knk)
...
[this section](/auth/jwt/tutorials/index.mdx) of recipes.
