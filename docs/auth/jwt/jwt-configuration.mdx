...
```json
{
  ...
  "sub": "optional-string-to-verify-subject",
...
```text
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..HL8jbe3FTkhiFckHsATMZVnAYtmYkI2TjnZ6pfFewcc
```

**Note:** `x-hasura-default-role` and `x-hasura-allowed-roles` are mandatory, while the rest of the claims are optional.

[See here for the JWT debugger](https://jwt.io/#debugger-io?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..HL8jbe3FTkhiFckHsATMZVnAYtmYkI2TjnZ6pfFewcc)
...
1.  Hasura Engine will fetch the JWK and will -
    ...
    3. third, check if an `Expires` header is present (if `Cache-Control` is not present), and try to parse the value as
       a timestamp.

2.  If it is able to parse `max-age`, `s-maxage` or `Expires` successfully, then it will use that parsed time to refetch
    the JWKs again.

    If it is unable to parse those values, then it will not refresh the JWKs (it assumes that if the above headers are
    not present, the provider doesn't rotate their JWKs). If the parsed time is less than a second, the JWKs will be
    fetched once per minute regardless.
...
1.  While Hasura Engine is running with refreshing JWKs, in one refresh cycle it will -
...
2.  If it is able to parse any of the above successfully, then it will use that parsed time to refetch the JWKs again.
    If it is unable to parse any values, then it will sleep for one minute and then start another refresh cycle.
...
You can specify the key name in the `AuthConfig` object, inside which the Hasura specific claims will be present, e.g.
...
The value of a claim referred by a JSON path must be a string. To use the JSON path value, the path needs to be given in
a JSON object with `path` as the key and the JSON path as the value:
...
using various JWT issuers. This configuration option allows for greater flexibility in authentication.
...
Refer to the [Auth0 JWT tutorial](/auth/jwt/tutorials/1-auth0.mdx) for a detailed guide on integrating Auth0 with
Hasura.
...
Refer to the [Clerk JWT tutorial](/auth/jwt/tutorials/4-clerk.mdx) to set up authenticated requests to Hasura with
Clerk.
