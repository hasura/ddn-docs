---
sidebar_position: 2
sidebar_label: JWT Configuration
---
...
- In JWT mode on a secured endpoint JWT authentication is **enforced** when **both** the `X-Hasura-Admin-Secret` header
  **and** the `Authorization: Bearer <JWT>` header are found in the request allowing _posing_ as a user with the
  specific role defined in the JWT.
...
- Auth0 publishes their JWK url at: `https://<YOUR_AUTH0_DOMAIN>.auth0.com` but has a known bug.
  [See Auth0's issues here](#auth0-issues).
...
`connection_init` when the websocket is connected to Hasura GraphQL Engine and is valid until the expiry of the JWT when
in JWT mode.
