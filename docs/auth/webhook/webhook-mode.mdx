...
reference for `AuthHookConfig` [here](/reference/metadata-reference/auth-config.mdx#authconfig-authhookconfig). For the
sample configuration above, the webhook will receive the `Authorization` and `Content-Type` headers in the body of the
request:
...
raw strings here using `value` instead of `valueFromEnv` and passing the keys.
...
and not forward any other headers. This will help in reducing the size of the request body and improve the performance
of the webhook.
