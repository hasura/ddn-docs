...
In [basic examples](auth/webhook/webhook-mode.mdx#update-authconfig), we're using the `BearerAuthorization` method. As
such, as we can make a request to our Hasura DDN instance by including a header with the key-value of
