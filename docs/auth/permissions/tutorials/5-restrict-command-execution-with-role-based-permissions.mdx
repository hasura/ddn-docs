...
This added role grants permission to the `author` role to execute the `UpdatePostsById` command but enforces a condition
that ensures a user can only update posts if the `id` of the post matches the `x-hasura-user-id` session variable passed
in the header of the request, restricting updates to their own posts.
