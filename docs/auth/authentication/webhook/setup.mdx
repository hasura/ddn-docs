...
## Step 1. Shaping the request and response
...
In the example below, we're demonstrating the authentication webhook that ships with Hasura for local development. You
can see that we're hitting the `validate-request` endpoint with a POST request.
...
Models and
Types a user has access to with each request via [ModelPermissions](/supergraph-modeling/permissions.mdx#modelpermissions-modelpermissions) and [TypePermissions](/supergraph-modeling/permissions.mdx#typepermissions-typepermissions).
