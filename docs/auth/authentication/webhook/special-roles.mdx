...
[updating up your `auth-config.hml`](/auth/authentication/webhook/setup.mdx#step-2-update-your-authconfig), shape your
...
:::info Your webhook's responses should vary by the role

When designing or implementing an auth server, it is crucial to create parsing logic which responds with different
values depending on the user's role. This approach ensures that each response contains the appropriate permissions and
information for its intended use.

By tailoring the response to the specific role, you enhance security and ensure that each user has access only to the
...
### Step 2. Create ModelPermissions

For whatever [Models](/supergraph-modeling/models.mdx) you'd like to publicly expose, create a
[`ModelPermission`](/supergraph-modeling/permissions.mdx#modelpermissions-modelpermissions) and provide a name for the
public role that matches the name of the role in the response above.
...
### Step 3. Create TypePermissions

Then, determine which [Types](/supergraph-modeling/types.mdx) you'd like to publicly expose by creating
