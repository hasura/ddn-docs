...
**You can read more about the fields and shape of the AuthConfig object in the
[modeling section of documentation](/supergraph-modeling/auth-config.mdx#authconfig-authhookconfig).**
...
In the example below the `x-hasura-is-owner` and `x-hasura-custom` are examples of custom session variables which will
be available to your permission rules in Hasura Engine.
