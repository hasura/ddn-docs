...
- Querying a `Customer` and also getting the analytics of their app usage from another data source.
- Querying an `Order` and also getting a live currency conversion for the value of the order enabled with a lambda data
  connector with a connection to a currency exchange API.

## Lifecycle

The lifecycle in creating a relationship in your metadata is as follows:
...
Relationships can be added between any kind of semantically related models and/or commands. They do not need to be
...
These tutorials follow on from the tutorials in the How to Build with DDN section.
...
#### Between a model and a business logic function as a command

To create a relationship between the `Users` model and the `shoutName` function, we can use the following command we can
create the following relationship. Using the
[VS Code extension](https://marketplace.visualstudio.com/items?itemName=HasuraHQ.hasura) makes the authoring of the
relationship easier.
...
Using the console, we can now query the `Users` model and get the `shoutName` function as a result.
