...
read operation or to run some custom business logic and can be exposed as queries or mutations in the GraphQL API.

For Lambda connectors, eg: (Typescript, Go, Python, etc) this will be a function (read-only) or procedure (mutation or
other side-effects) that also be exposed as a query or mutation in the GraphQL API.
