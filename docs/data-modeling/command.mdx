...
For example if you have a [native mutation](/connectors/mongodb/native-operations/native-mutations.mdx) that inserts
a user you can modify it to include a second field on new documents.
...
Delete the native mutation from your connector configuration.

```sh title="Delete native mutation in your connector's directory:"
rm <my_subgraph>/connector/<connector_name>/native_mutations/insert_user.json
```

Remove the command from your subgraph's metadata.
