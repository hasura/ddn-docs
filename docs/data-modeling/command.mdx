...
For classic database connectors this will be native query code for that source. This can be for example a more complex
read operation or to run some custom business logic. These can be exposed as queries or mutations in the GraphQL API.

For Lambda connectors this will be a function (read-only) or procedure (mutation or other side-effects) that also be
exposed as a query or mutation in the GraphQL API.
...
#### Via native operations {#via-native-operations-tutorials}
