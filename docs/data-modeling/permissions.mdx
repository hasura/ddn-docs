...
Permissions allow you to control what can be accessed in your API by which user roles.

When an authentication mode is enabled, the Hasura engine will look for session variables on every API request. The
engine can then use permissions defined in metadata and the session variables to determine if the request is allowed to
proceed.

To learn about authentication modes and supplying the session variables, see
...
If you no longer need a particular permissions, simply remove it from the relevant `ModelPermissions`,
`TypePermissions`, or `CommandPermissions` object.
