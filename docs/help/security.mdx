...
- **User access information:** Data on which users have access to the supergraph and subgraphs.

:::note

Connection string information from connectors is not sent to or stored within Hasura's control plane.
...
environment variables, is immutable. Critically, **secrets incorporated into the build via environment variables are not
directly exposed or retrievable from the built metadata once on the control plane**.
