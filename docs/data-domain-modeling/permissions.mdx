...
Below, we're creating a set of `ModelPermissions` on the `Author` model for the
roles `admin`, `user_1`, and `user_2`. The `Author` model has a object
relationship with `Article` model named as `Article`. The `Article` model has a `ModelPermission`
defined for `user_3` role which allows access to all `Article` rows.
          ...
          relationship:
            ...
            predicate:
              fieldComparison:
                ...
                operator: _eq
                value:
                  sessionVariable: x-hasura-article-id
```
