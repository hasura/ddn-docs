...
- `ModelPermissions` define which objects or rows within the model are allowed to be accessed by a role.
...
| `permissions` | [`[TypePermission]`](#typepermission)         | true     | The permissions object for this type.                           |
...
| Field      | Type                                            | Required | Description                                                |
| ---------- | ----------------------------------------------- | -------- | ---------------------------------------------------------- |
| `role`     | `String`                                        | true     | The name of the role on which you're creating permissions. |
...
| Field           | Type       | Required | Description                                                        |
| --------------- | ---------- | -------- | ------------------------------------------------------------------ |
| `allowedFields` | `[String]` | true     | List of field names (of the type) that are accessible to the role. |
...
| permissions | [`[ModelPermission]`](#modelpermission) | true     | The permissions for this type.                            |
...
Below, we're creating a set of `ModelPermissions` on the `Articles` model for the roles `admin`, `user_1`, and
`user_2.

As the `select` object for `admin` role is empty, it will return all rows. The `user_1` role's `select` object has a filter predicate that will return
rows where the `author_id` field is equal to the `x-hasura-user-id` session variable. The `user_2` role's `select` object has a filter
...
| `permissions` | [`[CommandPermission]`](#commandpermission) | true     | The permissions object for this command.                     |
