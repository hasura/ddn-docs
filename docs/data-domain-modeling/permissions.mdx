...
Below, we're creating a set of `ModelPermissions` on the `Album` model for the
roles `admin`, `user_1`, and `user_2`. The `Album` model has a object
relationship with `Artist` model named as `Artist`. The `Artist` model has a `ModelPermission`
defined for `user_3` role which allows access to all `Artist` rows.
...
The `user_3` role's `select` object has a filter predicate that will return rows
where the `article_id` field from the `Article` relations  is equal to the
`x-hasura-article-id` session variable.

<Thumbnail src="/img/permissions/album-artist-er.png" alt="Album Artist ER diagram" width="50%"/>
