...
sidebar_position: 2
sidebar_label: Types
description: Understand OpenDD types in Hasura, including primitive types, container types and custom types. Learn to define the structure of your data and map existing data source scalars to types in your data domain.
...
or arrays. Nullability in your data source types are represented using the `Nullable` container type.
...
[//]: # "TODO: Add reference to scalars 👇 when data sources are documented"
...
Below, we define a `UUID` type that is represented as a `String`:
...
Here, we define a `Tag` type that is represented as a `String` and a `Tags` type that is represented as an array of
tags of type `Tag`:
...
Below, we define an `author` type that has three fields: `author_id`, `first_name`, and `last_name`. Each field is
...
1. Mapping a `text` scalar type from the `my_source` source to a primitive `String` type.
