...
| Custom        | Such as `NewType` or `ObjectType`              |
...
In DDS, you can type alias by defining a DDS object of kind of `NewType`. To define a `NewType` you need to define a
name and an optional representation. This `representation` field can refer to any existing primitive, container or
...
1. A simple string representation of a NewType named `UUID`.
...
2. **A NewType used as array representatio of another NewType**: A NewType named `Tags` with an array representation of
   another newType `Tag`.
...
3. A newType named `OpaqueDate` without any representation
