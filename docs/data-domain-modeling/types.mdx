...
Type references are types that can contain other primitive or custom types. Currently, OpenDD supports nullable and
...
Primitive types supported by OpenDD are `ID`, `Int`, `Float`, `Boolean` and `String`.

Type references in OpenDD follow [GraphQL type syntax](https://spec.graphql.org/June2018/#sec-Combining-List-and-Non-Null).
Fields/arguments are nullable by default. To represent non-nullability in them, specify a `!` after the type name.
Similarly, array fields/arguments are wrapped in `[]`.
...
In the OpenDD spec, you can alias types by defining an object with `kind: ScalarType` and `version: v1`.
The object `definition` should include `name` and an optional `graphql` field.
...
This object has one field `typeName` that corresponds to the GraphQL type name to use for this scalar type.
...
| `type` | `String` | true     | Type of the field. |
...
This object has one field `typeName` that corresponds to the GraphQL type name to use for this object type.
...
GraphQL API. This object has one field `typeName` that corresponds to the GraphQL type name to use for this object type.

```yaml
graphql:
  comparison_expression_type_name: <GraphQLTypeName>
```
