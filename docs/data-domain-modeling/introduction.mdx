...
description:
  ...
  metadata generation and enhanced data source management. Understand the OpenDD spec structure, data sources, types,
...
metadata:
  - kind: HasuraHubDataSource
    # ... (other properties for HasuraHubDataSource)
  - kind: DataSource
    # ... (other properties for DataSource)
  - kind: AuthConfig
...
GraphQL API, etc. The data source will use a [Native Data Connector](/connectors/overview.mdx) to connect to the data
source and fetch the data.

### The HasuraHubDataSource object

The [`HasuraHubDataSource`](./data-sources.mdx) object allows you to specify a data source directly from the
[Hasura Hub](https://hasura.io/connectors/).
