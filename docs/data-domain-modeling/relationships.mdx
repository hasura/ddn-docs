...
| `namespace`        | `String`                                | false    | The namespace of the target model. Defaults to the namespace of the relationship metadata object. |
...
Syntactically there is no difference between local (Eg. a relationship between two columns in the same datasource, or an
existing foreign key relatioship) and remote (a relationship across two different data sources) relationships. Hasura
DDN will automatically detect the type of relationship and make the optimal query plan accordingly.

:::

### Relationship across Namespaces

Now Imagine if the Article model is defined in a differnt namespace called subgraph_articles and the Author and the
following Array relationship is part of the default namespace, we will now need to specify the target namespace in the
target section of the relationship definition as `namespace: subgraph_articles` as shown below:
...
Now Imagine you want to query author from the Article model which is specified in the subgraph_articles namespace, you
will need to specify where the Authors can be queries from by specifying the namespace in the target section of the
relationship: in this case specifying the namespace as `namespace: default` as shown below:
