...
A relationship allows you to define the relationships from a [**type**](./types.mdx) to a [**model**](./models.mdx) in
your data domain through which you will be able to query related data in one efficient request. For example, a
relationship can be defined from `Manufacturers` to `Products` so that you can query all the products for a given
manufacturer in one nested query.
...
:::info No distinction between local or remote data relationships

The OpenDD spec doesn't distinguish between local or remote data relationships. If you have data sources present on a
Hasura v3 project, Hasura will automatically identify this and make efficient queries based on that distinction.
...
To create a relationship, you need to define an OpenDD object with "kind": "Relationship", which has the following
fields:
...
The target configuration defines the target model and the type of relationship and can be represented as follows:
...
The relationship type defines the type of relationship between the source and target whether it is a Object (one-to-one)
or a Array (one-to-many) relationship and can be represented as follows:
...
The relationship mapping defines how the source and target should be connected and can be represented as follows:
