...
description:
  'Expand your understanding of OpenDD Relationships in Hasura, learn how to define linked or nested data queries,
  create relationships between types and models/commands. Explore mapping relationships with in-depth annotated examples
  and explore connecting data across namespaces.'
keywords:
...
[models](./models.mdx) or related [commands](./commands.mdx).
...
1. Relationship to retrieve the `Author` of an `Article` (here the command `get_author` takes in the `author_id` of the author
   and returns the `AuthorFromCommand`):
