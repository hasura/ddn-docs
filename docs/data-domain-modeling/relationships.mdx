...
description: "Expand your understanding of OpenDD Relationships in Hasura, learn how to define linked or nested data queries, create relationships between types and models. Explore mapping relationships with in-depth annotated examples and explore connecting data across namespaces."
keywords:
...
Now, imagine if the `Article` model is defined in a **different namespace** called `subgraph_articles` and the `Author`
and the following array relationship is part of the `default` namespace.

We will now need to specify the **target** namespace in the `target` section of the relationship definition as
`namespace: subgraph_articles` as shown below:
...
namespace. You will need to specify where the `Authors` can be queried from by specifying the **namespace** in the
`target` section of the relationship.

In this case, by specifying the namespace as `namespace: default` as shown below:
