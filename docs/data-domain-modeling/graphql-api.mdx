...
description: Learn how to use the Hasura GraphQL API, through model setups, proper usage of metadata structures, and handle GraphQL expressions. Make the best of Hasura's most powerful feature - generating GraphQL APIs for all your data.
...
Hasura generates GraphQL APIs for all your data.

With the generated API, you can query, insert, update and delete data in your data domain. The benefit of defining the
GraphQL API in the OpenDD spec format is that you can fully customize it to your needs.

:::info Currently supported features

Only query APIs are available in the current release.

:::

To enable the GraphQL API in Hasura, the following OpenDD spec metadata objects need to be defined:
...
For [Select Many](#select-many-queries) queries, it is necessary to have a `where` argument to filter rows. The `where`
argument is an input object which contains the model's type fields along with `_or`, `_and` and `_not` operators.
