...
- [modelGraphqlApi](#model-graphql-api): The root fields for a model which are exposed over the GraphQL API.
- [scalarTypeGraphqlExpression](#scalar-type-graphql-expression): Type names in the schema which are generated
  for data source scalars.
...
To create a root field for a model, define an object with `"kind": "modelGraphqlApi"`. Include the model's name
and at least one select-unique query API definition:
...
[//]: # (TODO: These are not done - these should be here and not in /graphql-api which is for the api usage docs )
[//]: # (Find examples of the `modelGraphqlApi` OpenDD spec object for:)
...
Defining a `scalarTypeGraphqlExpression` OpenDD spec object for a scalar type is completely optional. If not defined, the
