...
Hasura provides automatically generated GraphQL APIs for your data domain. The OpenDDS metadata is to define the data
domain, in way you get the flexibility to expose the required data, in the way you want.

You can define the data types, data models, relationships, permissions, and more using the DDS metadata to define your
data domain. Thus, your data domain can be agnostic on the shape of your data sources - It makes it so powerfull that a
data domain can continue to serve data in the same shape even after a source migration (say PostgreSQL to MongoDB or
moving column/s from one DB to another DB of different type), data distribution or any changes that you make on your
data store with minimal metadata changes in the OpenDDS metadata.

:::info Automatic Metadata generation with Console and CLI

This document will help you understand how the DDS Metadata is structured. however you will also be able to generate the
Open DD Spec automatically using the Hasura Console and CLI or you can chose to write the metadata manually and create
builds.

:::

A metadata is a array representation of OpenDDS objects, and a it would look like the following,
