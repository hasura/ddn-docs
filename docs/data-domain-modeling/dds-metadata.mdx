...
data domain can continue to serve data in the same shape even after a source migration (say postgres to mongo), data distribution (moving columns from one DB to another DB of different type) or any changes that you make on
your data store with minimal metadata changes in the OpenDDS metadata.
