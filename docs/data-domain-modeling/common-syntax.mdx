...
The `typemappings` are used by [models](/data-domain-modeling/models.mdx) and
...
| `fieldMappings` | [`FieldMappings`](#fieldmappings) | true     | The field mapping between the fields of OpenDDS output type and the NDC result_type   |


## FieldMappings {#fieldmappings}

The `fieldMappings` are used by [`typemappings`](#typemappings) to define mapping for the fields of the `outputType`.
