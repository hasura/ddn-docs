...
To create a `DataConnector`, start by defining an OpenDD object with the `kind` set to `DataConnector`. This object
should include three fields: `name`, `url`, and `schema` and an optional field `headers`.
...
`DataConnectorURL` is either a string containing the URL of the data connector or an object that defines separate
read/write data connector URLs:
