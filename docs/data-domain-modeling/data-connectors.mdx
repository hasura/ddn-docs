...
To create a `DataConnector`, start by defining an OpenDD object with the `kind` set to `DataConnector` and `version` set
to `v1`. This object `definition` should include four fields: `name`, `url`, `schema` and `headers`.
...
| `headers` | [`DataConnectorHeaders`](#dataconnectorheaders) | true     | Headers to pass to the data connector.                                                                                                 |
