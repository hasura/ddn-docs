...
A `DataConnector` is a way to specify where your data comes from and how it can be used. It can connect to various types of
...
Once you've declared a data conector, you can use it in your [model's](/data-domain-modeling/models.mdx) configuration by
...
To create a `DataConnector`, start by defining an OpenDD object with the `kind` set to `DataConnector`. This object should
include three fields: `name`, `url`, and `schema` and an optional field `headers`.
...
| `headers`            | [`DataConnectorHeaders`](#dataconnectorheaders) | false    | Headers to pass to the data connector.                                                                                                    |
...
`DataConnectorURL` is either a string containing the URL of the data connector or an object that defines separate read/write data connector URLs:
...
Read/Write URLs are used to access the appropriate data connector based on the GraphQL operation type. For example, if the
operation type is `Query` or `Subscription`, then the `read` URL will be used to access the data connector connector.
Similarly, for a `Mutation` operation, the `write` URL will be used. 

### DataConnectorHeaders

`DataConnectorHeaders` is an object that defines the headers to be forwarded to the data connector when making requests. They keys of this object are header names and the values are the header values. Eg: If a bearer authorization token needs to be forwarded, the headers field would be:
```yaml
headers:
