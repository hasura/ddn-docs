...
They are backed by functions or procedures declared in a `DataSource`. Commands are objects on which you will define and
configure user facing capabilities in your API. These will be extended in future releases according to the
[OpenDD Spec](https://github.com/hasura/open-data-domain-specification).

## Description

To create a command, you need to define a OpenDD object with `"kind": "Command"`, which has both `name` and `outputType`
fields.
