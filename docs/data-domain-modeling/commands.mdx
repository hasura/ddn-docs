...
description: "Begin executing business logic directly from your GraphQL API using Hasura Data Domain Specification commands. Explore examples, learn how to define objects, and discover the vital role of data connectors. Connect to REST endpoints, custom servers or serverless functions and better manage your back-end functions."
keywords:
  - hasura commands
  - hasura dds
  - graphql api
...
| `arguments`  | [`[ArgumentDefinition]`](data-domain-modeling/common-syntax.mdx/#argumentdefinition) | false    | The argument definitions for the command.              |
...
| `typeMappings`            | [`TypeMappings`](./common-syntax.mdx#typemappings) | false    | [Type](./types.mdx) mappings for the command from OpenDD types used in the command inputs/outputs to the corresponding data connector types. |
| `argumentMapping`         | [`ArgumentMapping`](#argumentmapping)              | false    | Mappings for the argument names of the command defined in OpenDD to their corresponding data connector names.                                |
...
(non-nullable `Int`) and returns an object of type `article` (similar to one defined [here](/data-domain-modeling/types.mdx/#object-type-examples)).
[The command is backed by a function](/data-domain-modeling/common-syntax.mdx/#typemappings) called `get_article_by_id` in the `db` data connector.
