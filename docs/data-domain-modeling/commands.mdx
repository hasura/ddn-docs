...
Commands are a convenient way to achieve any business logic you may need,
directly from your GraphQL API. You can use them to validate, process or enrich
some data, call another API, or log a user in. For eg: With Commands you can
connect to a REST endpoint which can be your own custom server, a public API, or
a serverless function. 

In the Open Data Domain Specification (OpenDD Spec), commands represent an
action that can be performed which returns back some result of a certain
[type](./types.mdx). 

They are backed by `functions` or `procedures` declared in a `dataSource`. These
commands are objects on which you will define and configure user facing
capabilities in your API. These will be extended in future releases according to
the [OpenDD Spec](https://github.com/hasura/open-data-domain-specification).
...
Arguments is a list of objects that defines the arguments for the command. An
argument object has the following fields:
...
Source is an object that defines the data source for the command. It has the
following fields:
