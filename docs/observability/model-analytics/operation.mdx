...
On the `Insights` panel from the left navigation, then on the `Model & Field Analytics` tab, operation analytics can be
...
The Model Requests section provides a visual representation of the number of requests made on a particular model in
terms of operations traffic.
