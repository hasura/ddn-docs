...
This helps in identifying usage patterns, most and least frequently accessed portions of your data graph. In turn helping optimize performance, manage resources better, and identify potential areas for improvement or scaling.

From the `Model & Field Analytics` page under `Insights` tab, operation analytics can be viewed for either an entire project or
a single build.
...
The Model Requests section provides a visual representation of the number of requests made on a particular model in terms of operations traffic.

The x-axis represents the time in days, while the y-axis represents the number of requests. Each bar in the graph
corresponds to the total number of requests made on a specific day.

In the provided graph, you can observe the distribution of requests over a month, with each bar indicating the number of
requests made on a given day. For instance, on July 8th, there was 1 request made on the model. Such insights help in
pinpointing peak usage periods and understanding how the model's usage varies over time.
...
- **Operation Name:** The name of the GraphQL operation.
- **Depth Level:** Indicates the nesting depth of the operation. Depth in graphql queries can be a performance
  bottleneck.
- **Operation Type:** The type of operation, such as query or mutation.
...
usage over time, enabling you to make informed decisions about your APIs evolution.
