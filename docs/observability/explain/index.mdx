...
In the example below, we are fetching the `Album` and its `Artist` and `Tracks`. We are also fetching the `Album` for
...
Now, from [the explain plan](/observability/explain/api-reference#example-1), we can understand that:

- There are three sequential steps:
  - Make a selection on the `Album` model
  - For each `Album`, make a selection on the `Artist` model
  - For each `Album`, make a selection on the `Tracks` model
- HashJoin the `Album` and `Tracks` models to construct the response
