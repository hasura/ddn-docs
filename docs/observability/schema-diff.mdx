...
The Hasura DDN provides a feature to compare two supergraph builds based on their GraphQL schemas. This feature helps
domain owners and API managers understand the impacts of changes quickly and effectively by comparing the schemas of two
supergraph builds.
...
## Using GraphQL Schema Diff feature in Console

1. You can go to [DDN console](https://console.hasura.io) and select a project. (Make sure you have a project with at
   least two supergraph builds if you don't have two supergraph builds, head to
   [Getting Started](/docs/3.0/getting-started/build/) to create builds to get started).
2. Select a build that you want to compare from the **Builds** header toolbar.
3. In the DDN project console, navigate to the **Insights** section from the sidebar
4. In the **Insights** section, navigate to the **Schema Diff** tab.
...
Imagine if you already have a supergraph build A applied to your Hasura DDN project and you want to see the impact of
applying a new supergraph build B. You can select build A as the source build and build B as the target build to compare
the schemas.

Which means if there are fields added in build B without removing any fields from build A, the schema diff will show
this as Safe changes and list them in the `Safe` section. If there are fields removed in build B, the schema diff will
show this as Breaking changes and list them in the `Breaking` section.
