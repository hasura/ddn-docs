...
- You can pass an `on_conflict` argument to convert the mutation to an
  [upsert mutation /mutations/postgres/upsert.mdx).
- You can return the number of affected rows and the affected objects (with nested objects) in the response.
- You can use the single object insert to get the inserted object directly as the mutation response.

See the [insert mutation API reference /api-reference/graphql-api/mutation.mdx#insert-upsert-syntax) for the full
specifications.
...
## Insert an object along with its related objects through relationships {#pg-nested-inserts}
...
3.  The array relationship objects are inserted at the end. i.e. in this case, the `articles` are now inserted with
    their `author_id` set to the author's `id` collected in the step 2. Hence, it's not possible to specify `author_id`
    in the data for the articles relationship.

:::info Note

The order of object insertion can be controlled using the
[insertion_order /api-reference/syntax-defs.mdx#objrelusingmanualmapping) option while creating a manual relationship.
This is necessary to ensure [nested inserts /schema/common-patterns/data-modeling/one-to-one.mdx#one-to-one-insert) in
...
Let's say the `articles` has a [many-to-many relationship /schema/common-patterns/data-modeling/many-to-many.mdx) with
...
2.  The array relationship objects (from the perspective of `article`) are inserted next i.e. the `article_tags` are
    ...
    2.  The parent object (from the perspective of `article_tags`) is inserted at the end i.e. the `article_tags` are
        ...
        the <em>article_tags</em> relationship.
...
[nested upsert caveats /mutations/postgres/upsert.mdx#pg-nested-upsert-caveats)).
...
[default value configuration /schema/postgres/default-values/postgres-defaults.mdx) i.e. via Postgres defaults or using
column presets.
