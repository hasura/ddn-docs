...
To convert an [insert mutation /mutations/postgres/insert.mdx) into an upsert, you need to use the `on_conflict`
argument to specify:
...
GraphQL Engine will automatically generate constraint names as enum values for the `constraint` field _(try
autocompleting in GraphiQL)_. Typically, the constraint is automatically named as `<table-name>_<column-name>_key`.
...
### Nested upsert caveats {#pg-nested-upsert-caveats}

:::info Note

The process by which nested inserts/upserts are executed is documented
[here /mutations/postgres/insert.mdx#pg-nested-inserts).
