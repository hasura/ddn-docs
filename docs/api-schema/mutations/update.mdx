...
- The `where` argument is compulsory to filter rows to be updated. See
  [Filter queries /queries/postgres/query-filters.mdx) for filtering options. Objects can be updated based on filters
...
permissions on at least the columns that make up the primary key as well as
update permissions on at least one column.
...
## Run multiple updates with different conditions{#multiple-updates}
...
<TOCInline toc={toc} filterTOC={'update-jsonb-columns'} />
...
<GraphiQLIDE
  query={`mutation update_extra_info {
  update_article(
    where: {id: {_eq: 1}},
...
**Example:** Delete the element at position 2 in the array value of the `jsonb` column `extra_info` of the `article`
table:
...
Hasura currently doesn't support nested updates. Follow
[this Github issue https://github.com/hasura/graphql-engine/issues/1573) for updates.
