...
## Model Select Permissions A Model Select Permission will need the model name and the role(s) for which the permission
## should be defined. For each role, you will need to define an optional filter expression. Objects (or rows/documents)
## that satisfy this filter predicate will be returned for that role.
