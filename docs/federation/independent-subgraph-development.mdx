...
[admins or developers](/hasura-cloud/collaborate/invite.mdx), allowing them to work independently on their subgraph and
data connectors without affecting other subgraphs. Subgraphs would also typically be named for and given responsibility
over the data domain they encapsulate, such as `users`, `orders`, `products`, etc.
...
1. As a supergraph admin, [create a supergraph normally](/getting-started/quickstart.mdx),
   [deploy](/deployment/deployment/index.mdx) and apply on Hasura DDN, as per the
...
4. Invite subgraph collaborators to the Hasura DDN project with
   [Subgraph Admin or Subgraph Developer permissions](/hasura-cloud/collaborate/invite.mdx).
...
1. [Accept the invitation](/hasura-cloud/collaborate/invite.mdx) to the Hasura DDN supergraph project.
...
3. On DDN, invite the subgraph project collaborators with [subgraph permissions](/hasura-cloud/collaborate/invite.mdx).
