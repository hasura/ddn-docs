...
### Step 2. Create a folder under the [registry](https://github.com/hasura/ndc-hub/tree/main/registry) folder  {#step-2}

To publish the `sqlite` connector under the `hasura` namespace, create the following directory structure:
...
### Step 3. Create a new directory in the registry

#### 3.1 Prepare connector files

Create a new folder structure in the `registry` directory of the `ndc-hub` repo:
...
Add the following files to your connector's folder:
...
#### Step 3.2 Create connector metadata
...
#### Step 3.3 Add initial connector version
...
3. Add [a `connector-packaging.json` file](#step-2) in this folder with the connector metadata.

#### Step 3.4 Submit for review
...
#### Step 3.5 Post-approval process
