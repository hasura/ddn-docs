...
- `<domain>`: Domain which will be used for accessing this ingress.  This could be constructed in the following format: **https://`<prod-api>`.`<your_fqdn>`** or **https://`<ddn-project-name>`.`<your_fqdn>`** (If you want to name it after the project name).
