...
## Step 10.  Create project API specific ingress {#create-project-ingress}
...
- `<v3-engine-helm-release-name>`: Helm release name for your v3-engine.  Choose an appropriate v3-engine Helm release name that you want to be applied to the project's API.
:::
...
## Step 11. Apply a build to project API {#apply-build}
...
In the previous step, you had created an ingress that is project API specific.  Here, you need to modify this ingress before we apply the build to the project API.  We will go ahead and do this via `kubectl patch`.
...
```bash title="kubectl patch project API ingress."
kubectl patch ingress/v3-engine -n <namespace> --type=json \
  -p='[{"op": "replace", "path": "/spec/rules/0/http/paths/0/backend/service/name", "value": "<v3-engine-helm-release-name>-v3-engine"}]'
```

Next, you need to go ahead and mark the build as applied.
...
## Step 12. View API via console {#view-api}
