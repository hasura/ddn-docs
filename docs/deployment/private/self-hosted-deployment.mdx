...
## Step 8. Deploy v3-engine with Helm (For build) {#deploy-engine}
...
## Step 9. Create ingress (For build) {#create-build-ingress}
...
## Step 11.  Create ingress (For Project API) {#create-project-ingress}
...
Repeat the Helm installation instructions in [Step 10](#deploy-engine-project-api).  Ensure that you are passing along the appropriate image tag for your v3-engine image (ie. The image tag which is associated with the specific build).
