...
## Step 3. Deploy connector(s) with Helm {#deploy-connectors}
...
## Step 8. Deploy v3-engine with Helm {#deploy-engine}
...
## Step 9. Create build specific ingress {#create-build-ingress}
...
## Step 10.  Deploy v3-engine with Helm (For project API) {#deploy-engine-project-api}

:::note Why am I deploying v3-engine again via Helm?

In this step, you will be deploying v3-engine that will be tied to your Project API.  This is needed so that we have a clear seperation between build specific workload deployments and the project API workload deployment.
:::
...
- `<v3-engine-project-api-helm-release-name>`: Helm release name for v3-engine project API.  This should be a unique release name which is specific to your project API deployment.
...
- `<project_api_observability_hostname>`: Observability hostname.  This will be the hostname for your project API (Note that you will be creating the ingress for this hostname in the next step).
...
## Step 11.  Create project API specific ingress {#create-project-ingress}
...
- `<domain>`: Domain which will be used for accessing this ingress.
              This matches the `<project_api_observability_hostname>` that was specified in [Step 10](#deploy-engine-project-api).
              This could be constructed in the following format: **https://`<prod-api>`.`<your_fqdn>`**.
...
Repeat the Helm installation instructions in [Step 10](#deploy-engine-project-api).  Ensure that you are passing along the appropriate image tag for your v3-engine image (ie. The image tag which is associated with the specific build which you want to apply).

After you go through the Helm installation, you need to go ahead and mark the build as applied.

:::info

- `<build_version>`: This was part of the output when you ran `ddn supergraph build create` command in [Step #6](#create-cloud-build).  You can also view the `<build_version>` history in the project's console view.
:::
...
## Step 13. View API via console {#view-api}
