...
By default, all Hasura DDN projects are in `Private` API Access Mode and protected by an `x-hasura-ddn-token`. You can
navigate to the project's settings and toggle the setting to `Public`. **Note: Your API will be accessible from any
source. You should protect it using either [JWT](/auth/authentication/jwt/index.mdx) or
[webhook](/auth/authentication/webhook/index.mdx) auth configuration.**
