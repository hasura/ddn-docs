...
In the following example we have a `supergraph-development.yaml` file for the `development` environment which specifies
a chain to the `subgraph-development.yaml` and the directory `metadata_development` to include for the development
metadata which sets the `noAuth` mode.
