...
- **Improved Performance:** A single connector instance can handle requests from multiple subgraphs, potentially leading
  to better connection pooling and resource utilization.
...
The key to connector reusability is the `DataConnectorLink` metadata object. Instead of each subgraph deploying its own
instance of a connector, different subgraphs (and even projects) can reference the same `DataConnectorLink`.

1.  **Deploy the Connector:** You deploy a single instance of the data connector, ideally in a region close to your data
    source.

2.  **Create the `DataConnectorLink`:** You create a `DataConnectorLink` object that points to this deployed connector
    instance. This object defines the connection details (URL, headers, etc.).

3.  **Reference the `DataConnectorLink`:** Within each subgraph that needs access to the data source, you use the
    `DataConnectorLink`'s `name` in the [Model](/reference/metadata-reference/models.mdx) and/or
    [Command](/reference/metadata-reference/commands.mdx) definitions.

## Example Scenario

Let's say you have a PostgreSQL database hosted in `us-east-1`. You have two teams: a "Products" team and a "Reviews"
team. Both teams need to access data from this database. Instead of deploying two separate PostgreSQL connectors, you
can deploy a single connector in `us-east-1` (close to the database) and reuse it across both subgraphs.

### Step 1: Deploy the Connector and Create the `DataConnectorLink`

1.  **Deploy the Connector:** Deploy a single instance of the `hasura/postgres` connector, for example in the
    `us-east-1` region. Let's say the deployment results in the following read and write URLs:

    ```
    Read URL:  https://my-postgres-connector-read.us-east-1.nhost.run
    Write URL: https://my-postgres-connector-write.us-east-1.nhost.run
    ```

    We'll need these values in the next step.
...
definition:
  ...
  url:
    readWriteUrls:
      read:
        value: https://my-postgres-connector-read.us-east-1.nhost.run
      write:
        value: https://my-postgres-connector-write.us-east-1.nhost.run
  headers: {}
