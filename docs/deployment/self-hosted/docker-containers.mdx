...
To begin, you'll need to create a set of project metadata files using the [DDN CLI](/reference/cli/index.mdx). We
...
docker build -t my-connector -f <subgraph_name>/connector/<connector_name>/.hasura-connector/Dockerfile.<connector_name> app/connector/<connector_name>/
```
