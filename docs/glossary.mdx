...
description: Discover the range of terms and definitions used within the Hasura DDN product. Find meanings for Data Delivery Network (DDN), serverless runtime, metadata authoring, and more.
keywords:
  - hasura ddn
  - serverless runtime
  - metadata authoring
...
In Hasura DDN, Namespaces provide autonomy, separation and security to teams for working with project metadata and allow
them to organize and structure metadata as per team and business requirements. There can be multiple namespaces in a
project.

All namespaces contribute to one build at any given time, ie: one namespace can be used create a build only when it does
...
for creating builds, deploying projects, creating tunnels and managing secrets. [Learn more](/ci-cd/cli/index.mdx).
