...
a per-request basis, enabling improved isolation and scalability. The serverless runtime also eliminates shared state
and cold start issues for enhanced performance.
...
tested independently. Builds exist in environments in projects and there is a one-to-many mapping between projects and
...
different stages of your development lifecycle. For example, you can create environments for `development`, `staging`,
`production` and personal development environments and then use them to contain different builds for each environment.
...
for creating builds, tracking objects, deploying projects and managing environment variables.
...
A environment variable (env var) is a key-value pair that contains small amounts of sensitive information such as
database connection strings. It is best practice to first create env vars using the Hasura CLI or Console and then use
them in metadata to avoid committing them to source control to potentially be leaked.
