...
enables blazingly-fast and secure delivery of real-time data. The new runtime engine in Hasura DDN accesses metadata
on a per-request basis, enabling improved isolation and scalability. The serverless runtime also eliminates shared
state and cold start issues for enhanced performance.
...
federated microservices execution model (loose coupling and scaling ownership).
...
web service endpoints that a connector needs to implement. [Learn more](/supergraph-modeling/data-connectors.mdx).
...
implements and can mutate data and have other side-effects. Each procedure has arguments and a return type.
...
tested independently. Builds exist in environments in projects and there is a one-to-many mapping between
projects and builds. [Learn more](/ci-cd/overview.mdx).
...
[Learn more](/ci-cd/config.mdx#supergraph-manifests).

## Environments

Environments allow you to organize your [project](/ci-cd/projects.mdx) metadata by creating different environments for
different stages of your development lifecycle. For example, you can create environments for `development`,
`staging`, `production` and personal development environments and then use them to contain different builds for each
environment.

Once you've created an environment you can then use it to create different builds by configuring a supergraph manifest
file for it. You'll then be able to access this environment's applied build via a different API endpoint.
...
for creating builds, tracking objects, deploying projects and managing secrets. [Learn more](/cli/overview.mdx).
...
This component is the underlying cloud service, which enables creating builds, applying builds and managing
environments. We consider this the brain of DDN Console and CLI – the component that drives most of the DX
functionalities.
