...
version 3 including a serverless runtime.
...
The process of defining and creating metadata files using the CLI, LSP, and Native Data Connector specifications in
Hasura DDN. It involves specifying data types, models, commands, data source and API configurations. Learn
[more](/supergraph-modeling/overview.mdx).
...
A function is a native data connector specification object and is a special type of collection. It only returns a single
row and a single column, can contain arguments and cannot be mutated.

Tracking a function results in the creation of a `Command` object in Hasura metadata.
...
not introduce a breaking change at the project level. [Learn more](/supergraph-modeling/overview.mdx).
