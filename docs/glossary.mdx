...
metadata objects like data domains, types, models, commands, data sources, and API configurations. Learn
[more](/data-domain-modeling/overview.mdx).

## Data Connectors:
A framework in Hasura DDN that allows developers to create custom data connector agents to integrate Hasura with
various external data sources and services. [Learn more](/native-data-connectors/overview.mdx).

## Model
A metadata object in Hasura DDN that defines the structure of a data source.
[Learn more](/data-domain-modeling/models.mdx).

## Relationships
A metadata object in Hasura DDN that defines the relationship between two models.
[Learn more](/data-domain-modeling/relationships.mdx).

## Metadata Build Service
A service in Hasura DDN which introspects data sources and can generate the corresponding metadata.
...
## Data Domains
In Hasura DDN, data domains provide autonomy to teams for updating project metadata and allow them to organize and
structure metadata as per team and business requirements. [Learn more](/data-domain-modeling/overview.mdx).

## Serverless Runtime System
The refactored runtime engine in Hasura DDN that fetches metadata on a per-request basis, enabling isolation and
improved scalability. It eliminates shared state and cold start issues for enhanced performance.

## Data Delivery Network (DDN)
An edge network in Hasura DDN that ensures low-latency, high-performance APIs globally. It enhances API performance
...
## Hasura Console
An interface in Hasura DDN that provides tools for metadata authoring, testing, deployment, and collaboration. It
offers an enhanced metadata editor and viewer to streamline API development.

## Push-Down Capabilities
The ability in Hasura DDN to delegate certain query operations to the underlying data source, improving query
...
## Metadata Authoring
The process of defining and creating metadata files using the OpenDD Spec in Hasura DDN. It involves specifying data
