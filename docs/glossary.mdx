...
metadata objects like data domains, types, models, commands, data sources, and API configurations. Native Data
Connectors (NDCs): A framework in Hasura v3 that allows developers to create custom data connector agents to
integrate Hasura with various external data sources, databases, and services.

## Model
A metadata object in Hasura v3 that defines the structure of a data source.

## Relationships
A metadata object in Hasura v3 that defines the relationship between two models.
...
## Data Sources
Any external data source, database, or service that can be connected to Hasura v3 using a Native Data
Connector (NDC) agent.
...
## Data Domains
In Hasura v3, data domains provide autonomy to teams for updating project metadata. They allow developers
to work on specific sections of metadata independently while contributing to a shared project without compromising
or overlapping on the work of other teams.

## Environments
Environments in Hasura v3 enable developers to work with independent infrastructure setups within a project. Each
team or developer can have their own environment with specific sets of secrets.

## Serverless Runtime System
The refactored runtime engine in Hasura v3 that fetches metadata on a request basis, enabling isolation and improved
...
commands for creating builds, deploying projects, creating tunnels and managing metadata.

## Metadata Authoring
The process of defining and creating metadata files using the OpenDD Spec in Hasura v3. It involves specifying data
types, models, commands, data sources, and API configurations.
