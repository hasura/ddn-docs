...
To set up GraphQL API on your data domain, you may need to define the following DDS metadata objects.

- [modelGraphqlApi](#model-graphql-api): Specify the root fields to be exposed over GraphQL API for a model.
- [scalarTypeGraphqlExpression](#scalar-type-graphql-expression): Customize type names of schema generated for data source scalars.
...
To define `modelGraphqlApi` object, you will need model's name and at least one select-unique query API definition.
Find the JSON schema for the object below.
...
:::info Note
It is unnecessary to define `argumentsInputField` and `argumentsInputType` if arguments are not defined on the model.
:::
...
For [Select Many](/graphql-api/queries.mdx#select-many) queries, it is necessary to have `where` argument to filter nodes.
The `where` argument is an input object contains model's type fields along with `_or`, `_and` and `_not` operators.
...
The `Post` model is backed by a source. The fields of the model are mapped to `source_post` object type as the following.
- `post_id` -> `source_post`.`id` of type `Integer`
- `content` -> `source_post`.`content` of type null `TEXT`
- `tags` -> `source_post`.`tags` of type `[TEXT]`

In the above case, only `post_id` and `content` fields are present in `where` argument input object.
You cannot filter objects using `tags` field as it is mapped to an array scalar.

Each field is an input object with comparison operators as fields. These operators are defined for each
scalar type in the data source's schema. <!-- TODO: refer data source schema docs when available -->
...
Using `scalarTypeGraphqlExpression` DDS object, you can provide custom type name for the comparison input object
of each scalar type.
...
:::info Note
Defining `scalarTypeGraphqlExpression` DDS object for a scalar type is completely optional. If not defined,
...
Defining a comparison object type name for `TEXT` and `Integer` scalar types.
...
Assuming the `TEXT` scalar type is exposing a `like` comparison operator and `Integer` scalar type is exposing `greater_than` and `less_than` operators.
...
```
And, the source scalar types are [represented](/api-schema/types.mdx#scalar-type-representation) as DDS types as follows.
- `TEXT` -> DDS `String`
- `Integer` -> DDS `Int`
...
If no `scalarTypeGraphqlExpression` object defined for `TEXT` and `Integer` scalar types, then the GraphQL schema is generated as
```graphql
