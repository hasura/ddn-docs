...
For relationship filtering to be processed efficiently, the data connector must support the `relations_comparison`
...
However, if the `relations_comparison` capability is absent, the query engine will handle the relationship comparisons at
...
processing, as more data needs to be transferred to the query engine for evaluation.
