...
sidebar_position: 5
description: Sort query results on PostgreSQL in Hasura
keywords:
  - hasura
  - docs
  - postgres
...
specifying `asc_nulls_first` and last on descending ordering by specifying `desc_nulls_last` enum value e.g. `{name:
desc_nulls_last}`.
...
Key order in input object for order_by is not preserved.
This means you should only have a single key per object, or you may see unexpected behavior.
