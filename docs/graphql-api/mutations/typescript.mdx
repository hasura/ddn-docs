...
# CQRS via the TypeScript Connector

## Introduction

**Command Query Responsibility Segregation (CQRS) is a pattern that separates read and write operations for a data
store.** This pattern is useful when you want to scale read and write operations independently.
...
const client = new Client(`<YOUR_CONNECTION_STRING_OR_ENV_VAR>`);
...
<Thumbnail
  ...
  alt="Insert user mutation using CQRS"
