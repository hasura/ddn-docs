...
sidebar_label: "II: Add business logic"
sidebar_position: 3
...
When we ran `ddn quickstart`, we told the CLI that we wanted to include the TypeScript connector. Inside of the
`/app/app_functions` directory we can find all the files the CLI generated, including `functions.ts` which will contain
our custom logic:
...
<Thumbnail src="/img/get-started/trackScoldUser.gif" alt="Track scoldUser command" width="1000px" />
...
export async function scoldUser(userId: string) {
  // Yes, for ease we're reusing the same sample app 😉
...
## Step 5: Create a relationship from a command to a model
...
- First, we introduced you to a quick and easy way of adding [custom business logic](/connectors/typescript.mdx) to your
  API. Hasura can host your TypeScript connector for you, or you can host it yourself. You can learn more about the
  connector and deployment options to Hasura DDN or self-hosted solutions by visiting
  [this page](/connectors/deployment).
