...
Once a project is defined, we can use the `dev` command to continually watch for changes, build the supergraph on Hasura DDN, and make the supergraph API available.
...
You can connect a local database by using a tool like [ngrok](https://ngrok.com/).
...
```bash
ddn create project --dir ./my-first-project
```
...
Additionally, it [scaffolds some files](/docs/ci-cd/projects.mdx) to get you started with working on your project.

## Step 4: Add a connector manifest {#step-4-add-a-connector-manifest}
...
Then, create a connector manifest by passing a name — in this case `mydb` — to the `add connector-manifest`
command:

```bash
ddn add connector-manifest mydb --hub-connector hasura/postgres --type cloud
```

The CLI will respond with information about our successfully-added connector manifest and alert us to our next steps:

```bash
INF connector "hasura/postgres:<VERSION>" with name "mydb" added successfully to Subgraph "app"
INF Please add your Postgres Connection URI as the value for CONNECTION_URI env var in app/mydb/connector/mydb.build.hml file
```
...
network from where it's hosted to your nearest Hasura region. We're using the `v3-docs-sample-app`'s connection string, but
also have other data sets available in this dropdown or, if you wish, you can add your own connection string.

<details>

<summary>See connection strings »</summary>
...
`MYDB_CONNECTION_URI` environment variable with the connection string under the `app` subgraph:

```yaml
supergraph: {}
subgraphs:
  app:
    MYDB_CONNECTION_URI: "postgresql://read_only_user:readonlyuser@35.236.11.122:5432/v3-docs-sample-app"
```

Next, update your `/app/mydb/connector/mydb.build.hml` file to reference this new environment
variable:
...
definition:
  ...
  deployments:
    - context: .
      env:
        CONNECTION_URI:
          valueFromEnv: MYDB_CONNECTION_URI
```

</TabItem>
<TabItem value="RawString" label="Raw Connection String">

Open your project in VS Code and open the `mydb.build.hml` file in our project. We can then add the
...
+---------------+----------------------------------------------------------------------------------------------------+
INF Starting ConnectorManifest watcher for connector "mydb" in subgraphName "app"
INF Starting ConnectorLink watcher for connector "mydb" in subgraphName "app"
```
...
Each [**project**](/ci-cd/projects.mdx) can have multiple builds, but only one can be applied to a the project's
endpoint. All builds on a project have a unique URL on Hasura DDN to access their GraphQL API which can be shared with
other users.
...
- `ddn update connector-link mydb --add-all-resources`
...
You can use the GraphiQL Explorer in Console to create your own query or write it manually.
...
[**Models**](/supergraph-modeling/models.mdx), defined in metadata, add collections from our data sources to the GraphQL API.
...
Let's see how easy it is to add authorization to your API!
