...
5. **For users on Windows, we recommend utilizing
   [Powershell](https://learn.microsoft.com/en-us/powershell/scripting/overview?view=powershell-7.4) for this tutorial.
   The tutorial commands are based on Unix/Linux syntax. However, PowerShell on Windows supports these commands through
...
We've provisioned a few read-only databases for you to use for this guide if you'd like. The connection strings are
available later in the guide.
...
A database filled with a number of functions.
...
If we run the command from the output of the previous step, we'll see the CLI creates our first build, displays the URL
for our project's Console, and watches for changes 🎉

```bash
cd my-first-supergraph && ddn dev
```
...
In just a few lines — and with the assistance of LSP, our underlying language server — we can declaratively restrict
...
Open your project directory in VS Code. Find the `/app/app_connector/models/users.hml` file. Then, with the guidance of
LSP, let's make our `ModelPermissions` object look like the metadata below in our `users.hml` file:
