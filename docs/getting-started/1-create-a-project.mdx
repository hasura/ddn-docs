...
During this process, we'll add a **data connector** to connect to a PostgreSQL data source. Data connectors can connect
to any data source. Connectors are available from Hasura and the community on the
...
Finally, the CLI will let us know we can run the following command to start watching for changes and building our API:
...
Additionally, the CLI **automatically** tracked the tables and and any foreign-key relationships in our database!
...
In just a few lines — and with the assistance of [LSP](https://microsoft.github.io/language-server-protocol/) fueling
...
This happened automatically! When we ran `ddn dev` after creating our project, the
[connector was built and deployed](/connectors/overview.mdx) automatically to Hasura DDN. Hasura can host these for you
or you can deploy them on your own infrastructure.
...
But, for now, let's keep moving and add another subgraph to our API!
