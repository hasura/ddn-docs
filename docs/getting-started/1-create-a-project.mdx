...
We'll utilize a local development workflow to iteratively test our connectors and update our project's configuration;
this requires the Docker daemon running in the background. Before proceeding, ensure the daemon is up.
...
We'll see the CLI creates our first build, displays the URL for our project's Console, and watches for changes 🎉
...
Using our extension — fueled by [LSP](https://microsoft.github.io/language-server-protocol/), our underlying language
server — let's track the different relationships in our source and have our API updated automatically.
...
In just a few lines — and with the assistance of LSP — we can declaratively restrict access so that a user is only able
...
We instantly added all foreign-key relationships from our data source to our API. This unlocks the potential for rich,
deeply-nested queries that reach across different models in our API.
