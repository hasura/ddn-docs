...
[exposing a model](/getting-started/connect-to-data/03-add-source-entities.mdx), we'll create a new local build of our
API.
...
ddn supergraph build local \
  ...
  --output-dir engine \
  --env-file my_subgraph:my_subgraph/.env.my_subgraph.local
```

This will create a build of your supergraph.

With this command we're specifying the supergraph we want to use to build locally with `--supergraph`, the
output directory name for the engine with `--output-dir`, and the environment variables needed for each subgraph to
build with `--env-file`.

Note that when building a supergraph there can be multiple subgraphs, so you need to specify the environment file
for each subgraph with the subgraph name as a prefix in the way we show above for the `--env-file` flag. The
`--env-file` flag can be repeated to take the environment file for each subgraph.
...
named `app`. Thus, the query above will return a response that looks like this:
