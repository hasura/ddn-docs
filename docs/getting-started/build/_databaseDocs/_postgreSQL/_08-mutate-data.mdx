...
We provide two main ways to mutate PostgreSQL data - Via auto-generated mutations, and via the TypeScript connector.
...
These auto-generated mutations are disabled by default by the PostgreSQL connector. In order to enable them,
...
Next, we will introspect the connector and import all of the newly available mutations.
...
as a pre-set argument in place of the permission checks for each command.
...
After editing the `.hml` files of your models, build the supergraph:
