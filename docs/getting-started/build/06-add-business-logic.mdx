...
- Make sure your port in `.env.local` and the one referenced in the `compose.yaml` file `ports:published` field match.
- Add the Node.js Lambda connector compose file to the main compose file in the root of the
  project if you want them to start all together. Eg:
