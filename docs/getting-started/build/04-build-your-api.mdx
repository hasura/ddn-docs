...
each. We can either specify the environment **file** with the subgraph as a prefix as we're doing here with
`--subgraph-env-file` or we can use the `--subgraph-env` flag to specify **individual environment variables** in the
command itself eg: `my_subgraph:key=val`. Both `--subgraph-env-file` and `--subgraph-env` flags can be repeated to
specify multiple environment files or variables.
