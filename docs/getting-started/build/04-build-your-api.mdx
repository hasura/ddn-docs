...
command itself eg: `--subgraph-env my_subgraph:key=val`. Both `--subgraph-env-file` and `--subgraph-env` flags can
be repeated to specify multiple environment files or variables.
