...
```bash title="Run the following command, updating the referenced paths to match your desired directory structure"
...
This command simply adds a new key-value pair to a context file in a project's `.hasura` folder. The key is `subgraph`
and the value is the path to the `subgraph.yaml` which you just passed.
