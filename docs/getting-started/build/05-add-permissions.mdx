...
By adding `modelPermissions`, we've enabled access to the `Users` model for the `user` role. As shown in the screenshot
above, this means that _only_ the `Users` model is available to a consumer with the `x-hasura-role` value of `user`.
...
from accessing irrelevant system information, such as when users were last seen.
