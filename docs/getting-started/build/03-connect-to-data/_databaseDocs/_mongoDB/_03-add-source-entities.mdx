...
```bash title="For example, with a collection of movies"
...
When adding a model, it is critical to use the same casing as the `collection` from your connector's metadata file.

:::

<details>
  <summary>
    Each time you add a model, you'll see a new named file in the `metadata` directory of its subgraph. For example, if
