...
- A [MongoDB connector](/getting-started/build/03-connect-to-data/01-connect-a-source.mdx) initialized, introspected and
  running in Docker.
...
Let's add and name the `hml` file the same as our connector, `my_mongo`:
...
ddn connector-link add my_mongo \
  ...
  --configure-host http://local.hasura.dev:8083 \
...
```env
MY_SUBGRAPH_MY_MONGO_READ_URL="http://local.hasura.dev:8083"
MY_SUBGRAPH_MY_MONGO_WRITE_URL="http://local.hasura.dev:8083"
```
