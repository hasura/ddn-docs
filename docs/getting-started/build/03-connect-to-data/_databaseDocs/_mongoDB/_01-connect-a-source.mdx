...
connector specific configuration file, and generate the necessary Hasura metadata which describes our API by creating
...
ddn connector introspect my_mongo --add-all-resources
```
...
Additionally, the CLI generated and updated a `DataConnectorLink`. Hasura DDN uses a concept called "connector linking"
to take configuration JSON files for a data connector and transform them into an `hml` (Hasura Metadata Language) file
as a [`DataConnectorLink`](/supergraph-modeling/data-connector-links.mdx) metadata object which describes our data
source.
...
## Step 3. Create a new build and restart the services
