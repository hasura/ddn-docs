...
- A [ClickHouse connector](/getting-started/03-connect-to-data/connect-a-source) initialized and introspected
...
We have already generated the schema representation using the `ddn connector introspect` command, and now we're
about to use that to create the `DataConnectorLink` `hml` object with the `ddn connector-link add` and then `update`
commands.

The `DataConnectorLink` is a metadata object which describes any data source in the Hasura Metadata Language (HML)
format.
...
The new file is scaffolded out at `my_subgraph/metadata/my_clickhouse.hml`.
...
[exposing source entities](/getting-started/build/03-connect-to-data/add-source-entities).
