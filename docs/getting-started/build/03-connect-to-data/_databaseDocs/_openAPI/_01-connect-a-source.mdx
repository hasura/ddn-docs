...
facilitate the connection. Then, with the DDN CLI, we introspect the OpenAPI document to generate Typescript functions,
and then use those to create DDN metadata which defines our API.
...
## Step 4. Track your functions

Functions generated from the OpenAPI spec are represented as [Commands](/supergraph-modeling/commands.mdx) in your API.
The next command we'll run will take each generated function and create an `hml` file for it. These files will then be
