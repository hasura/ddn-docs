...
```text
NDC_OAS_BASE_URL=https://petstore3.swagger.io/api/v3/openapi.json
```
...
- `NDC_OAS_FILE_OVERWRITE` (optional): A Boolean flag to allow previously generated files to be overwritten.
Defaults to `false`.
...
to regenerate files. If you make changes to the `api.ts` file, you can run the `connector introspect`
command again to regenerate the files again. They will only be overwritten if the `NDC_OAS_FILE_OVERWRITE` environment
