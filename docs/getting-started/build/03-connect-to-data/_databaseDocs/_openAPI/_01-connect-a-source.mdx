...
Let's restart our docker compose services so that Docker can create a OpenAPI connector build for us now that we've
added and introspected the connector.

```bash title="From the root of your project, run:"
HASURA_DDN_PAT=$(ddn auth print-pat) docker compose up --build --watch
```

This starts our Hasura Engine, observability tools and the OpenAPI connector all together since we added the
