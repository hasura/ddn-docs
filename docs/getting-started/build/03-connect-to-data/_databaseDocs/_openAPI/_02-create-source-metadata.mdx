...
commands.
3) Finally, use the `ddn command add` and `ddn model add` commands to create the command and model objects which
define our API.
...
can run the `update` command to have the CLI look at our `functions.ts` file and generate our
`DataConnectorLink` metadata in `hml` format:
...
With a data connector fully configured, you can now start to create metadata for each endpoint in our OpenAPI
document in `hml`. Learn how to do this by
