...
The CLI understands the hostname `local.hasura.dev`, which functions as an alias for `localhost`. Here's how it works:

- On the **host machine**: `local.hasura.dev` resolves to `127.0.0.1` using the public DNS of your host machine. This
  means that when you use this hostname, the CLI and connector plugins recognize it as pointing to your local machine.

- Inside a **Docker container**: `local.hasura.dev` resolves to `host-gateway` using the `extra_hosts` configuration in
  your Docker Compose setup. This ensures that the hostname correctly points to the host machine from within the
  container.

In essence, `local.hasura.dev` acts as an alias for `localhost`, working seamlessly both on your local machine and
