...
for convenience and clarity sake for this tutorial, but it can be anything you want.
...
- A `schema.json` file which is the JSON schema that `configuration.json` follows. This is to enable autocomplete in
VS Code.
...
With the connector configured, we can now use the CLI to introspect our PostgreSQL database and create a data
connector specific configuration file. This configuration file, which describes the schema of our PostgreSQL
database, will then be used in a later step to generate Hasura metadata which describes our API.
...
Let's restart our docker compose services so that Docker can create a PostgreSQL connector build for us now that
