...
`admin` role. However, you can easily add authorization rules — what we term permissions — by declaring them in metadata.
This allows you to control what data can be accessed and what fields can be returned by your API based on the user role,
or any other value in session variables.
...
business logic solution. In this guide, we're going to use the `jwt-cli` to encode and generate a new token with the
different claims to run a test of our new permissions.
...
#### Step 3.2. Set up your AuthConfig object
