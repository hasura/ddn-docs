...
<Thumbnail src="/img/get-started/ERD/create-relationships.png" alt="Create relationships" width="1000px" />
...
- Existing models or commands in your supergraph which can be related to an `ObjectType`. For more information on
  ...
  [adding data source entities to the API](/getting-started/build/03-connect-to-data/02-create-source-metadata.mdx) for
  your data source. And for commands you can see the
  [integrating business logic](/getting-started/build/06-add-business-logic.mdx) section.
...
are located together. We can use the [IDE plugin](/getting-started/build/00-prerequisites.mdx#install-lsp) to help us in
...
- We know that when we query this relationship on an order, we want it to be named: "customer". (Hit `tab` to go the next editable placeholder.)
...
- We know that the relationship will be 1-to-1. As in, an order only has one related customer. So the `relationshipType` will be an Object not an Array.
- We know that on the source `Orders` `ObjectType` there is a `customerId` field that maps to the id field `customerId` on the
  target `Customer` `Model` which we want to use to define the relationship.
...
Check out information on how to [mutate your data](/getting-started/build/08-mutate-data.mdx) with Hasura DDN.
