...
ddn connector init my_pg --subgraph my_subgraph --hub-connector hasura/postgres --configure-port 8082 --add-to-compose-file docker-compose.hasura.yaml
```
...
- A `docker-compose.my_pg.yaml` a file to run the MongoDB data connector locally in Docker. Includes the
specified port.
