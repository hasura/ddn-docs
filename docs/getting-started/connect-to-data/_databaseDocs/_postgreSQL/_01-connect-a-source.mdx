...
- A `configuration.json` file which contains configuration info for the data connector itself.
- A `schema.json` file which is the JSON schema that `configuration.json` follows.
