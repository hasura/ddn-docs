...
```bash
ddn connector init my_mongo --subgraph my_subgraph --hub-connector hasura/mongodb --configure-port 8082 --add-to-compose-file docker-compose.hasura.yaml
```
...
We're specifying the port to run the connector on using the `--configure-port` flag. This is important to avoid port
collisions with other connectors or services which you might have running on your machine. Remember to use a
different port for each connector you may have running.

**Add to compose file**

We're specifying the `--add-to-compose-file` flag to add the connector's own Docker compose file to the main
`docker-compose.hasura.yaml` file. This allows us to start the Hasura Engine and connectors together.
...
- A `docker-compose.my_mongo.yaml` a file to run the MongoDB data connector locally in Docker. Includes the
specified port.
