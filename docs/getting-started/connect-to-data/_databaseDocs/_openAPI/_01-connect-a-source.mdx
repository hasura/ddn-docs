...
Once we've generated these functions from our OpenAPI document, we can modify them to add our own additional business
logic to them.
...
base URL of the API. Eg: `NDC_OAS_BASE_URL=https://petstore3.swagger.io/api/v3` This will be used in the `functions.ts`
file which we'll generate in the next step to make API calls.

You can also edit this value directly at the top of the `functions.ts` file manually later. Eg:
```typescript
const api = new Api({
