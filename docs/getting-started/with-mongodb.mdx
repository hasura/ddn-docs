...
project, connect it to a new MongoDB database, and run your first query.
...
```sh title="Next, you'll create a new Hasura DDN project. Run the following command:"
...
| ---------------------- | -------------------------------------------------- | -------- |
| `MONGODB_DATABASE_URI` | `mongodb://username:password@hostname:port/dbname` | Yes      |

:::info Connecting a local database?

Hasura DDN uses a unique domain to resolve local services. In the connection string above, you can replace `hostname`
with `local.hasura.dev` if you're connecting to a local database.
...
```sh title="Now, you'll track the collections in your MongoDB database:"
