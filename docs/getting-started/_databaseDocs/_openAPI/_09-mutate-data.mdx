...
- A [OpenAPI connector](/getting-started/connect-to-data/connect-a-source) initialized
- A [DataConnectorLink](/getting-started/connect-to-data/create-source-metadata) configured

:::

We can add business logic to our functions to modify data before it's sent to the API, or before it's returned to the
client.
...
If we use the "@save" tag in the JSDoc comments, changes we make to the function will not be overwritten if we
re-introspect the connector.

:::info JSDoc comments

The OpenAPI connector uses [JSDoc comments](https://jsdoc.app/) to determine how to expose a function: either as a
...
`@allowrelaxedtypes` converts unsupported types into scalars because certain types, such as `any` don't map
well to GraphQL. So, `@allowrelaxedtypes` tells the Node.js connector to convert these into an opaque scalar type
instead of rejecting the function that uses them.

:::

Or for instance we could add a completely **new** function to get a pet by its ID and get a capitalized name:
