...
You can choose our `hasura/postgres` connector which by default gives you a docker compose file to run a local Postgres
database. You can check this guide to [set up a local Postgres database](/connectors/postgresql/local-postgres.mdx) using Docker.
Or you can connect to our sample Postgres database using this URL - `postgresql://read_only_user:readonlyuser@35.236.11.122:5432/v3-docs-sample-app`
