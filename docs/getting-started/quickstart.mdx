...
MongoDB, MySQL, ClickHouse, etc.). We're also passing the name `my_connector` for the connector which is referenced in
subsequent commands. When you run this command with the interactive flag (`-i`), you'll be prompted to select a
data connector and enter the connection details for your data source. The CLI will add a
