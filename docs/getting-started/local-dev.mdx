...
We've organized your types in the `pg_db` directory and your models in the `models` directory. Each model has its own
file and is named after the table it represents.
...
:::tip See a failed build error?

If this is the case, head to any of the `hml` files in your project and save it without making any changes. This will
trigger a new build and the error should go away.
...
## Step 8: Track the function as a procedure
...
:::tip Code action not showing up?
...
## Step 9: Execute the function using GraphiQL
