...
Open your empty `metadata.hml` file and start typing `HasuraHubDataConnector`. Hit `Enter` and the extension will
...
We'll also need to add an [`AuthConfig`](/auth/authentication/auth-config.mdx). Hasura requires this to keep your API
secure; you can use JWTs or webhooks. For testing, you can use Hasura DDN as your webhook provider. Simply add the
following to the bottom of your `metadata.hml`:
