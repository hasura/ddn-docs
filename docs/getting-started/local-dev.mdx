...
```bash
hasura3 build create --project ./hasura.yaml --description "Connect Datasource and track entities"
```
...
```bash
hasura3 build apply --project-id <PROJECT_ID> --build-id <BUILD_ID>
```
...
```bash
hasura3 build list --project-id <PROJECT_ID>
```
