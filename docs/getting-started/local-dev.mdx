...
description: "Kickstart your Hasura project from scratch by using VS Code with our new CLI, connect to a PostgreSQL database, and execute your initial query. This guide unveils new Hasura concepts, such as builds and our novel metadata structure to enrich your GraphQL application development process."
keywords:
  - hasura ddn
  - graphql
  - setup hasura
...
:::info Which port should I use?

The `<PORT>` argument is the address on which your local function is running. From **Deno**, you should see an output,
...
```bash
# e.g., hasura3 tunnel create http://0.0.0.0:08100
hasura3 tunnel create <SOCKET-AND-PORT>
```
...
You'll use this endpoint to form a connection string for your function, like this:

```text
http://<TUNNEL_ENDPOINT>
```

Inside of your `commands.hml` file, replace the locally running connector with the `<TUNNEL_ENDPOINT>` value returned by
the CLI:
...
definition:
  ...
  url:
    # highlight-start
    singleUrl: "http://<TUNNEL_ENDPOINT>"
  # highlight-end
```

This will allows Hasura DDN to access your locally running function via the tunnel.
