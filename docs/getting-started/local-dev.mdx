...
Most of the workflow involved talking to Hasura Cloud, so the first step is to login to Hasura Cloud on the CLI.
...
use the link shown in the terminal to launch the flow. 
...
to create a new Hasura DDN project on the Cloud as well.

```bash
hasura3 init --dir hasura-ddn-project
cd hasura-ddn-project
```
...
The next step is to add a data source to the project so that we can build APIs on that source. With Hasura DDN, 
...
becuase Hasura can create tunnels to connect to local databases.
