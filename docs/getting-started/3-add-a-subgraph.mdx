...
sidebar_label: "Add another subgraph"
sidebar_position: 4
...
A [connector manifest](/connectors/overview.mdx) is the file which contains the details of the connector's
configuration. This tells Hasura DDN what capabilities of the connector should be utilized and how to build your
connector for your data source.

:::

Then, in our `.local.yaml` in the root of the project, we can add the new subgraph and connection string:
...
We can then run `ddn dev` to generate a new build and introspect our Chinook data source.
...
You can bring this up by pressing `Shift+Ctrl+P` on Linux or Windows and `Shift+Cmd+P` on macOS. Be sure to select
`chinook_connector` and `Default` from the options that appear from the palette's dropdown menu:
...
music-based dataset that contains a type called `Artist`. We have a `favorite_artist` colum on our `users` table in the
`app` subgraph.
...
:::info Remember, you can use LSP to assist in authoring metadata
...
We created a second subgraph called `chinook` and added a completely separate data source. We then created a
relationship across these subgraphs to link disparate tables and make them available via a single query. We can even
visualize this using the console's explorer, available in the left-hand navigation.
...
deploy connectors, and create rock-solid CI/CD pipelines for easy and automated deployments.
