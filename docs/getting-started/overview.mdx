...
A valid supergraph is compiled into an immutable [build](/project-configuration/builds), which powers the resultant
API. The API build is run by the [Hasura Engine](/glossary.mdx#immutable-build-runtime-system) and can be hosted on the
...
browsing the supergraph, it's subgraphs and the data sources which connect to them. On the Console you're also able
to track and trace the performance of your API.
