...
Hasura DDN defines this entire API in a metadata format which conforms to the
...
A valid supergraph is compiled into an immutable build, which enables the resultant API. In turn, the API build
is run by the [Hasura Engine](/glossary.mdx#immutable-build-runtime-system) and can be hosted on
...
[permissions](/supergraph-modeling/permissions.mdx), and [relationships](/supergraph-modeling/relationships.mdx)
across data.

Subgraphs can correspond to distinct data sources, or to a specific data domain. They have their own development
lifecycle and allow different teams to work on different parts of the supergraph at the same time without stepping
...
Hasura DDN also includes a Console for querying the API as well as a visual explorer for browsing subgraphs and their
data sources which connect to it. On the Console you're also able to track and trace the performance of your API.
