...
The following variables were generated with **each** initialized data connector for local development, and are
overwritten with the appropriate values for a cloud deployment when building your connectors and supergraph:

- `APP_MY_CONNECTOR_AUTHORIZATION_HEADER`
- `APP_MY_CONNECTOR_READ_URL`
- `APP_MY_CONNECTOR_WRITE_URL`

Additionally, Hasura DDN will override the following values at runtime, so they can be left blank in the `.env.cloud`
file:

- `APP_MY_CONNECTOR_HASURA_CONNECTOR_PORT`
- `APP_MY_CONNECTOR_OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`
- `APP_MY_CONNECTOR_HASURA_SERVICE_TOKEN_SECRET`
