...
If you want to deploy your connector to a specific data center region you can use configuration in your
`Connector` object in metadata as below:
...
definition:
  ...
  source: hasura/connector:<version>
  context: .
  envFile: .env.cloud
  #highlight-start
  regionConfiguration:
    - region: <region> # choose one of these supported regions: https://hasura.io/docs/3.0/graphql-api/multi-region-routing#supported-regions
      mode: ReadWrite
  #highlight-end
```
