...
you, ensuring rapid delivery of data from your API to your consumers. **For each Python connector in a project,
...
ddn connector build create \
  --connector my_subgraph/connector/my_python/connector.cloud.yaml \
  --target-env-file my_subgraph/.env.my_subgraph.cloud \
  --target-subgraph my_subgraph/subgraph.yaml \
  --target-connector-link my_python
```
