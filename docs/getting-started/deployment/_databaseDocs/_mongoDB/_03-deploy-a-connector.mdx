...
As we did with the `env.local` file, we need to update the `env.cloud` file with any connection URI we need for the
cloud hosted connector to be able to communicate with the MongoDB data source.
...
ddn connector build create \
  --connector my_subgraph/connector/my_mongo/connector.cloud.yaml \
  --target-env-file my_subgraph/.env.my_subgraph.cloud \
  --target-subgraph my_subgraph/subgraph.yaml \
  --target-connector-link my_mongo
```
