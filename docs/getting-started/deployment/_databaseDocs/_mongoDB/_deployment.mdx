...
ddn connector build create --connector ./my_subgraph/connector/my_mongo/connector.cloud.yaml
```
...
this example with a connector named `my_mongo` in the `my_subgraph` subgraph.

```env title="For example, my_subgraph/.env.cloud.my_subgraph:"
MY_SUBGRAPH_MY_MONGO_READ_URL=<cloud-connector-read-url>
MY_SUBGRAPH_MY_MONGO_WRITE_URL=<cloud-connector-write-url>
```
