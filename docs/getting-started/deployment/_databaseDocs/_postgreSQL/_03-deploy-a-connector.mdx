...
We need to update the `env.cloud` file with the connection URI we need for the cloud hosted connector to be able to
...
:::tip Deploy to a multiple regions

If you want to deploy your PostgreSQL connector to a multiple data center regions you can use configuration in your
`Connector` object in metadata as described in our [multi-region routing guide](/graphql-api/multi-region-routing.mdx#supported-regions).
