...
Here we may need to update the `env.cloud` file with the connection URI we need for the cloud hosted connector to be
able to communicate with the PostgreSQL data source.
...
If you've been running a local PostgreSQL instance, you can replace the connection string with a cloud
instance connection string or set up an [ngrok](https://ngrok.com/) tunnel to a local instance.
