...
Create a `.env.cloud` environment variable file in the same directory as your `.env.local` file for the connector. **The
keys in this file should be identical to the keys present in the `.env.local`, but may have different values**. For
instance, in the next step, we'll build the cloud connector and use the returned URLs for the `read` and `write`
...
If you haven't already deployed a connector for a subgraph, you'll need to create a `.env.cloud.my_subgraph` file in
