...
You'll need to [deploy the connector](/getting-started/deployment/deploy-a-connector/) so its availalbe in your cloud project.
...
```bash
$ ddn subgraph build create --subgraph --subgraph another_subgraph/subgraph.yaml --env-file --subgraph another_subgraph/.env.another_subgraph.cloud
```
