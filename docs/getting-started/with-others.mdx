---
sidebar_position: 6
sidebar_label: With other sources
description: "Learn the basics of Hasura DDN and how to get started with other sources."
keywords:
  - hasura ddn
  - graphql api
  - getting started
  - guide
---

import Prereqs from "@site/docs/_prereqs.mdx";

# Get Started with Hasura DDN and other Sources

On the [Connector Hub](https://hasura.io/connectors), you can find information about our wealth of data connectors.
These data connectors allow you to connect nearly any data source to Hasura DDN and build an API on top of it.

We recommend using the [Quickstart](/getting-started/quickstart.mdx) and referencing the individual connector's docs
from ☝️ to get started with a connector-specific setup.
