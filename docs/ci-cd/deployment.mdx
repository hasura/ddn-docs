...
You'll also need to create build profiles for each environment by creating new named files in the root of your
directory:
...
#### Step 3: Reference build profiles in `hasura.yaml`

Then, inside of your `hasura.yaml`, you can reference these build profiles:
...
environment can be created using the Hasura CLI, and passing `build-profile-staging.yaml` to the `--profile` flag. Then,
...
The same process as `staging` is followed, except `build-profile-prod.yaml` is used as the argument passed to the
`--profile` flag.
