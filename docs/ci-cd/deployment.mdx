...
environment can be created using the Hasura CLI, and passing `staging` to the `--environment` flag. Then, they can apply
the build using the `build version` returned by the CLI.

The team can run any of their tests against this new build before heading to production.

### Production

The same process as `staging` is followed, except `production` is used as the argument passed to the `--environment`
flag.
...
You can drop this [GitHub Action](#) into your repository and configure it to run on merges to specific branches to
