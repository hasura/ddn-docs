...
Here, you can see that the `build-profile.yaml` file contains `version` and `spec` sections. The `version`
...
This tells the Hasura CLI to assign metadata from `subgraphs/default` directory to the `default` subgraph` when creating a new build on the Hasura DDN project.
...
`compatibility-config.hml` files. The `auth-config.hml` file defines the authorization configuration for your supergraph i.e. the configuration for how the queries to your Hasura DDN project should be authorizated. The `compatibility-config.hml` file defines the compatiblity date of your supergraph metadata. You'd likely not want to change the compatiblity config unless you're sure about it.
...
While it's not recommended, you could choose to deviate from the default directory structure that the Hasura project initializes for you. For example, you could have a single file with all the metadata objects, including your `HasuraHubDataConnector` objects, models, and commands, as long as you specify that file to be in your subgraph resources in the build profile However, this would be difficult to
manage and maintain.

With a more advanced configuration we can leverage subgraphs and break apart our supergraph into modular compentents where we have multiple `hml` files stored in their respective subgraph directories.
