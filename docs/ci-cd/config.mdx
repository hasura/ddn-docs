...
Your `hml` file contains a list of metadata objects according to the OpenDD spec. In the example above, we could add a
...
This lets Hasura DDN know that we have two build profiles, and that the `build-profile_staging.yaml` is the default
to use when creating a new build.

### metadata.hml

In our basic example, we only had one `hml` file. With a more advanced configuration we can leverage namespaces
and break apart our supergraph into team-ownable components where we have multiple `hml` files.
...
Here, we're simply defining our `CompatibilityConfig` and `AuthConfig` objects. These are using Hasura DDN's
...
If we wanted to separate our models into a different file, we could use a `newmodel.hml` file in the `Subgraph1`

If there are multiple teams working on the same data source, we could use a `metadata2.hml` file in the `Subgraph2` to
