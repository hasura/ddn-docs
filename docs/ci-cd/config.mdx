...
file must conform to the [supergraph modeling](/supergraph-modeling/overview.mdx) and [subgraph rules](./subgraphs.mdx).
...
Your `hml` files contain metadata objects. In the example above, we could add a `HasuraHubDataConnector` object to
connect to a data source. Then, import our tables as models and create a build on Hasura DDN.
...
objects, models, and commands, as long as you specify that file to be in your subgraph resources in the build profile.
...
The contents of the `hml` files can be that of any valid metadata object as long as they conform to the rules of
[subgraphs](./subgraphs.mdx).
