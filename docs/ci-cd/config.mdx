...
This is the entry point to a Hasura project. This specifies the project name and lists the build profile files
as well as a default build profile to use if one is not specified when running a CLI command.
...
You can create additional build profile files for different environments you may need eg: `staging`, `production`,
...
### Subgraphs of hml metadata files
...
This tells the Hasura CLI to assign metadata from `subgraphs/default` directory to the `default` subgraph` when creating
a new build on the Hasura DDN project.
...
You can see that this file contains a `version` and a `project` section. The `version` section is used to
...
`compatibility-config.hml` files. The `auth-config.hml` file defines the authorization configuration for your supergraph
i.e. the configuration for how the queries to your Hasura DDN project should be authorized. The
`compatibility-config.hml` file defines the compatibility date of your supergraph metadata. You'd likely not want to
change the compatibility config unless you're sure about it.
...
The `AuthConfig` object is used to configure authorization for your data supergraph. You can learn more about
...
You could also choose to arrange your subgraphs by data type, eg: `subgraphs/default/users`,
