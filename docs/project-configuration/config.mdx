...
subgraph config files to use for building the supergraph
...
These config files tell Hasura DDN how to construct your subgraph. It will contain information such as which
metadata resources to use for the build.
...
- `includePaths` specifies the directories and files where metadata for the subgraph can be found.
...
By default, the CLI generates a file called `subgraph.yaml` for a new subgraph but any number of
subgraph config files can be created and referenced in CLI commands.
...
By default, the CLI generates two files - `connector.cloud.yaml` and `connector.local.yaml` but any number of
...
- `.env.globals.cloud` and `.env.globals.local` files contain all the environment variables, if any, which are
required by the **globals** subgraph's metadata objects for cloud and local deployments respectively.
