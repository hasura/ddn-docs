...
It is also possible to split this file into different .env:
...
- `envMapping` declares the environment variables needed by the subgraph and where to get it from. For example,
   ```
   POSTGRES_WRITE_URL:
...
- `envMapping` declares the environment variables needed by the subgraph and where to get it from. It works the same
  way as it does for subgraphs. Only env vars declared in the `envMapping` are passed into the subgraph.
...
is used by default by all local development commands (if other env files are not specified) and `cloudEnvFile` is used by default 
by all cloud commands (like `ddn supergraph build create`).
...
This file is configured by the `ddn context` set of commands.
