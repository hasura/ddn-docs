...
Add a new key: `CONNECTION_URI=<pg_url>` to the file: `<subgraph_name>/metadata/connector/<connector_name>/.env.local`

TODO: add example block
...
```
ddn connector introspect –connector <subgraph_name>/metadata/connector/<connector_name>/connector.yaml
```
