...
A new revision of the [DDN CLI](/cli/installation.mdx) has been released. With this release the local project directory
structure created by CLI versions `<= v2024.05.23` is now deprecated.
...
:::note No changes to existing DDN projects
...
Below are steps to convert a local project created with the DDN CLI versions <= `v2024.05.23` to the new project structure.
...
Copy the file containing the functions from your existing project directory to
`<subgraph-name>/connector/<connector-name>/functions.ts` in the new project directory. This file should typically
