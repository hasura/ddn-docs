...
to the supergraph in the "parent" project.

In this setup, one or more subgraphs are defined in each repository and depending on the role of the user on the cloud
project, can be built and applied to the supergraph independently.

You can read more about [collaboration](/collaboration/index.mdx) and [federation](/federation/overview.mdx) in the
relevant sections.
