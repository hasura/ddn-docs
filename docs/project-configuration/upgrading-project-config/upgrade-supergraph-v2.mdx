...
- The `AuthConfig`, `GraphqlConfig` and `CompatibilityConfig` metadata objects are now not defined as supergraph globals
  ...
  only 1 of these objects can be defined across the supergraph, but they can now be defined in any subgraphs based on
  convenience. By default, these are now added to a new **globals** subgraph.
...
```bash
ddn codemod upgrade-supergraph-v1-to-v2 --dir <project-dir-path>
```
...
based on the supergraph objects and setting up the new **globals** subgraph.
