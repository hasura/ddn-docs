...
A new revision (**v1.0.0**) of the [DDN CLI](/cli/installation.mdx) has been released with some changes to the
...
Note that this is a CLI-only change and does not impact Hasura DDN projects. You can continue using existing
DDN projects and builds and also be able to create new builds on them using the new CLI revision
(with the new local project structure) and also the older CLI revision (with the old project structure).
...
OpenTelemetry collector config does not affect your local dev by setting the field 
`$.services.otel-collector.image` in the `docker-compose.hasura.yaml` file to `otel/opentelemetry-collector:0.104.0`.
