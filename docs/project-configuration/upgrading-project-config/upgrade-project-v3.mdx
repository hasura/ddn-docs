...
- The CLI now doesn't add the subgraph name as a prefix to metadata objects by default.
...
### Step 5: Add metadata generator prefix (optional)

The CLI now doesn't add the subgraph name as a prefix to metadata objects by default. You can keep the earlier behaviour
by updating the generator config in your Subgraph configs.

```yaml title="For example, app/subgraph.yaml"
kind: Subgraph
version: v2
definition:
  name: app
  generator:
    ...
    #highlight-end
  envFile: .env.mysg.local
```

### Step 6: Create top level .env files (optional)
