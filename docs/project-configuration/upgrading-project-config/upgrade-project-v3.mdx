...
Additionally, you can now set env files for local development and for cloud builds in the context via the `localEnvFile`
and `cloudEnvFile` keys. These values will be used for local and cloud builds by default.
...
#### Step 4.3: Create top level .env files
