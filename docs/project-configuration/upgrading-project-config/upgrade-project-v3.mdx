...
- The `Subgraph` object is upgraded from `v1` to `v2`.
  - The env variables requited to build the subgraph now need to be explicitly defined in an `envMapping` property.
  - The connectors in the subgraph and their corresponding connector links can now be defined in the subgraph config to
    simplify interactions. (TODO: elaborate)
- Common workflows have been simplified to make developing with the CLI easier. (TODO: elaborate)
- The CLI command arguments and flags have a few changes. See details on the new commands [here](/cli/commands/ddn). (TODO: elaborate)

We recommend you go through our [new quickstart guide](/getting-started/quickstart.mdx) to learn about
