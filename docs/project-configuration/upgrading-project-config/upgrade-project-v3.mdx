...
You can now set env files for local development and for cloud builds in the context via the `localEnvFile`
and `cloudEnvFile` keys.

**If you have a shared supergraph config file for local and cloud**, both these env files can then be set into your context
to allow managing both local and cloud deployments using the same context. i.e.
...
the CLI now doesn't generate individual env files for subgraphs and connectors by default and instead uses the
common top-level env files.
