...
the CLI now doesn't generate individual env files for subgraphs and connectors by default and instead uses
common top-level env files.

The env files for local development and for cloud builds can be set in the context via the `localEnvFile`
