...
The default naming convention for GraphQL types/fields has changed. The CLI now doesn't add the subgraph name as a
...
The `envMapping` field defines how any provided environment variables are mapped to the environment variables
expected by the connector / subgraph.
...
```bash
# set context with cloud supergraph config e.g. cloud
ddn context set-current-context cloud
...
#### Step 3.1: Update engine build dir
...
##### Step 3.1.1: Move engine build assets
...
##### Step 3.1.2: Update engine compose file
...
#### Step 3.2: Add metadata generator prefix

As the CLI now doesn't add the subgraph name as a prefix to the generated GraphQL types and fields by default. You can
keep the earlier behaviour by updating the generator config in your Subgraph configs.
...
definition:
  ...
  generator:
    ...
    #highlight-end
  envFile: .env.app.local
  ...
```

#### Step 3.3: Create top level .env files
