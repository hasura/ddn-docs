...
The DDN CLI allows you set values for commonly used flags in the project context to avoid having to repeat them with
every command.

The project context config is stored in the `.hasura/context.yaml` file in the project directory.

On project directory creation, a context called `default` is initialized with the following values by default:
...
[//]: # ()
[//]: # (The following hierarchy is followed to determine the value of particular key:)

[//]: # ()
[//]: # (Flag > Context > Config file)

[//]: # ()
[//]: # (i.e. the value passed via CLI command flag takes precedence over the value set in the context which takes precedence)

[//]: # (over the value set in a config file)
...
| `supergraph`   | the supergraph config file to use for the operation                          | `--supergraph`                    |
| `subgraph`     | the subgraph config file to use for the operation                            | `--subgraph`                      |
