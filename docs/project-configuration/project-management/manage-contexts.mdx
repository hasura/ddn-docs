...
[`create-context` CLI command](/cli/commands/ddn_context_create-context.mdx).

For a full list of context-related commands, refer to [this category of commands](/cli/commands/ddn_context.mdx).
Although you can modify these values directly in the file, we recommend using the CLI for consistency and ease. Examples
are provided below.
...
also override this manually using [the `ddn context set` command](/cli/commands/ddn_context_set.mdx).
...
[use the CLI](/cli/commands/ddn_connector_env_add.mdx).
