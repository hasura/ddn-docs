...
The `match` field is used to match the regex to the request path, while `matchMethods` specifies the HTTP methods to
...
Also, note that we can not use the pre-defined DDN endpoints (like `/graphql`, `/v1/explain`, `/healthz`, `/metrics`,
etc.) in the `match` field.
