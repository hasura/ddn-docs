...
not match DDN's pre-defined endpoints (such as `/graphql`, `/v1/explain`, `/healthz`, `/metrics`, etc.).
...
The `pre-route` plugin can be used to add custom endpoints to DDN. This can be used to add custom endpoints to DDN that
are not part of the pre-defined DDN endpoints. Some use cases are:
