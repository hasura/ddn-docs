...
not match DDN's pre-defined endpoints (such as `/graphql`, `/v1/sql`, `/v1/rest`, `/v1/explain`, `/healthz` and
`/metrics`).

[See the reference here](supergraph-modeling/engine-plugins.mdx#lifecyclepluginhook-lifecyclepreroutepluginhook) for
pre-route plugin metadata configuration.
