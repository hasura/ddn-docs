...
Let's take an example where the engine is confiured with multiple pre-parse plugins: `Pre-parse hook 1`, `Pre-parse hook
2`, `Pre-parse hook 3` ...

<Thumbnail src="/img/plugins/multiple-pre-parse-plugins.jpg" alt="Adminer" width="750px" />

In this example, the engine is configured with three pre-parse plugins. The engine sends the request to `Pre-parse hook
...
<Thumbnail src="/img/plugins/multiple-pre-parse-plugins-p1-response.jpg" alt="Adminer" width="750px" />

If all the pre-parse plugins return a `Continue` response, the engine continues with the execution and sends the response
generated by the engine to the client.
