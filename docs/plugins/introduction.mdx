...
graph RL
    ...
    Pre_Response_Hooks -->|Response| Client
...
is sent back to the client. Any plugins defined after the one that halts execution will not run.
...
The engine stops further execution and sends the response to the client. Plugins configured after the one that halts
execution will not be called.
