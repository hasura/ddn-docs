...
If the database is running locally, then the database should be made accessible to the internet using tools like [ngrok](https://ngrok.com/downloads/linux)
[ngrok](https://ngrok.com/downloads/linux). For example,
...
CREATE TABLE users (user_id int, name varchar(255), age int);
```
