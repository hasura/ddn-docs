...
```plaintext
Server=127.0.0.1,1433;Database=master;Uid=sa;Password=Password123;
```
...
```sh title="Use the mssql-tools shell to start the SQL prompt:"
docker exec -it sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U SA -P 'Password123' -d 'master'
```

:::note

Add the flag `-C` is to apply the `trust server certificate` set to true. This shoud only be used for local database and not on production databases.
...
Application such as [Microsoft Azure Studio](https://azure.microsoft.com/en-us/products/data-studio) can also be used to seed the database
...
In `app/connector/my_sqlserver/configuration.json`, you'll see schema updated to include operations for the `posts` table. In
`app/metadata/my_sqlserver.hml`, you'll see `posts` present in the metadata as well.
