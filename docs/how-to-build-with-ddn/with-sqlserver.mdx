...
sudo docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=Password!" -p 1433:1433 --name sqlserver -d mcr.microsoft.com/mssql/server:2019-latest
...
```plaintext
Server=127.0.0.1,1433;Database=master;Uid=sa;Password=Password!;
```
...
CREATE TABLE users (user_id int NOT NULL, name varchar(50) NOT NULL, age int NOT NULL);
```
