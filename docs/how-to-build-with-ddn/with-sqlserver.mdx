...
```plaintext
Server=<hostname>,<port>;Database=master;Uid=sa;Password=Password123;TrustServerCertificate=true
```

:::note

If the database is created using the local docker setup, it needs to be hosted on cloud using applicarions such as
...
Alternativey, other applications and tools can also be used to seed the database, such as
