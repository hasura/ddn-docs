...
The OpenAPI Connector internally makes use of the TypeScript Lambda Connector to expose the generated code as a GraphQL
API. Read more about the TypeScript Lambda Connector [here](../business-logic/add-a-lambda-connector.mdx).
...
You can import all APIs from your OpenAPI Document as commands by running `ddn command add my_openapi "*"`.
...
In your projects `.env` file, set the `APP_MY_OPENAPI_NDC_OAS_FILE_OVERWRITE` to `true`.
