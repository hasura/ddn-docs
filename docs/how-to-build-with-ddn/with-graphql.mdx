...
configuration for introspection vs. executing requests, see the configuration docs here.
...
```sh title="Let's the command to query for rockets:"
ddn command add my_graphql rockets
```

In `app/connector/my_graphql/config.json`, you'll see schema updated to include operations for the `posts` table. In
`app/metadata/my_graphql.hml`, you'll see `posts` present in the metadata as well.
...
As an example, you may have a PostgreSQL database with a `booster` model exposed and which contains a unique `id` field.
This field can be used to create a [relationship](/reference/metadata-reference/relationships.mdx) between the `rockets`
command and a `booster` in your database (or any other resource that utilizes a unique `id`) to return information
across your sources in a single query.
...
Take a look at our GraphQL connector docs to learn more about how to use Hasura DDN with existing GraphQL APIs. Or, if
