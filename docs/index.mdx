---
title: Hasura DDN Documentation
description: Hasura DDN documentation
keywords:
  - hasura
  - docs
  - manual
  - DDN
  - data
  - delivery
  - network
sidebar_label: Introduction
sidebar_position: 0
slug: index
hide_table_of_contents: true
---

import Link from "@docusaurus/Link";
import VersionedLink from "@site/src/components/VersionedLink";
import ArrowRight from "@site/static/icons/arrow_right.svg";
import Basics from "@site/static/icons/features/basics.svg";
import GraphQL from "@site/static/icons/features/graphql.svg";
import Beaker from "@site/static/icons/features/beaker-feature.svg";
import Logo from "@site/static/img/logo.svg";

# Hasura v3 Documentation

<div className="gallery">
  <div className="sub-heading">
    <div className="front-matter">
      <p>
        Welcome to Hasura v3's public beta!
        <br />
        <br />
        We're happy to have you with us and are excited to share what we've been working on.
        <br />
        <br />
        <b>
          Hasura v3 is a Data Delivery Network (DDN) that provides instant GraphQL APIs on <i>all</i> your data.
        </b>{" "}
        It's a new way to think about building applications and is the fastest way to connect data sources and generate
        an API on top of them.
        <br />
        <br />
        Hasura brings you lighting-fast builds, a declarative metadata specification, and a set of developer tools to make
        you and your teams more productive than ever.
        <br />
        <br />
        With Hasura's data supergraph, you can combine data from multiple sources to build powerful APIs that can handle
        complex queries and relationships. The supergraph allows you to integrate data from your databases, existing
        microservices, and external APIs, so you can build modern, scalable applications with ease.
        <br />
        <br />
        This documentation is a work in progress and we would love your feedback. If you have any suggestions or find any
        bugs, please use our handy feedback component on each page and reach out.
      </p>
      <div>
        <Logo />
      </div>
    </div>
  </div>
  <div className="sub-heading">
    <h3>Popular Topics</h3>
  </div>
  <div className="card">
    <div className="card-header">
      <Basics />
      <h4>Basics</h4>
    </div>
    <p>
      Here, you can get up and running with Hasura and our core concepts; we'll cover all the basics so you're ready to
      start building.
    </p>
    <VersionedLink to="/basics/overview">
      Learn the basics <ArrowRight className="arrow" />
    </VersionedLink>
  </div>
  <div className="card">
    <div className="card-header">
      <Beaker />
      <h4>Get Started</h4>
    </div>
    <p>
      Whether you prefer a GUI or the CLI, we've got you covered. Learn how to get started with Hasura and deploy a
      GraphQL API on your data in minutes.
    </p>
    <VersionedLink to="/quickstart/">
      Deploy your first project <ArrowRight className="arrow" />
    </VersionedLink>
  </div>
  <div className="card">
    <div className="card-header">
      <GraphQL />
      <h4>GraphQL API</h4>
    </div>
    <p>We provide an instant GraphQL API by generating your GraphQL schema for you based on your data.</p>
    <VersionedLink to="/graphql-api/overview">
      Learn about the GraphQL API
      <ArrowRight className="arrow" />
    </VersionedLink>
  </div>
</div>
