...
This tutorial assumes you're starting from scratch and using the PostgreSQL image that ships with the connector, but you
...
### Step 1. Authenticate your CLI {#step-2}
...
acknowledge your login and check for any updates.
...
Once you move into this directory, you'll see your project scaffolded out for you.

### Step 3. Initialize your PostgreSQL connector {#step-4}

```sh title="Initialize a new PostgreSQL connector:"
...
```sh title="Next, introspect your PostgreSQL database:"
ddn connector introspect my_pg
```

After running this, you should see the `users` table present in your `app/connector/my_pg/configuration.json` file.
...
Hasura Metadata Language file to bring the `users` table from PostgreSQL into your API.
...
### Step 11. Iterate on your schema
...
In `app/connector/my_pg/configuration.json`, you'll see the new `posts` table. In `app/metadata/my_pg.hml`, you'll see
the new `posts` table present in the metadata as well.
...
```sh title="Since there's already a foreign key on the posts table, we can easily add the relationship:"
...
can explore more advanced features like [permissions](/supergraph-modeling/permissions.mdx) and more.
