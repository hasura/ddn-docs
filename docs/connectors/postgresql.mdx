...
- An object with keys `reads`, and `writes`, each an object where each key maps a region to a list of database servers.
...
Currently only queries are supported, so supplying any `writes` serves has no effect.

:::

:::info Current support

Currently only a single database per region is supported, so only the first uri supplied will be used.
...
version: 1
connection_uris:
- postgresql://alice@database.host
metadata:
  tables: {}
  native_queries:
    artist_below_id:
