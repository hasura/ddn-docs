...
The MongoDB data connector makes available any database resource which is listed in its configuration. In
order to populate the configuration, the connector supports introspecting the database via the Hasura CLI `update`
command.
...
MongoDB data connector's configuration. They are similar to a "view" in a conventional database, but more
expressive as they allow parameters similar to functions and do not require any DDL permissions on the database. See
the configuration reference on [Native Queries](/connectors/mongodb/config.mdx#updating-with-introspection).
