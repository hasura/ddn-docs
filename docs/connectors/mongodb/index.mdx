...
connector and guide you through the configuration process within a Hasura project.

The MongoDB data connector makes available any database resource that is listed in its configuration. In order to
populate the configuration, the connector supports introspecting the database via the Hasura CLI `update` command.
