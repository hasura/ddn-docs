...
Native queries with the `function` representation have the restriction that the aggregation pipeline output must be
a singe document with a single property named `__value`. When the function is invoked the document will be unwrapped,
and the value in `__value` will be used as the return value of the function. This allows functions to produce values
that are not objects, as opposed to MongoDB aggregation pipelines which can only produce streams of objects.
