...
:::tip
Using a representation of function in MongoDB Native Queries follows the same requirements of [functions in DDN
spec][DDN: functions]. Specifically:

> A function is a collection which returns a single row and a single column, named __value
<br/>
Refer to the [function example](#example-using-representation-function) above.
:::
...
Determines how the pipeline is ran. If `"inputCollection"` is present then the pipeline runs with that collection as the
starting point. The same as doing `db.artist.aggregate` in mongosh. If no `"inputCollection"` is present then the
pipeline is ran with db as the starting point. The same as doing `debugger.aggregate` in mongosh.
