...
:::tip Connnection Environment Variable

To set up the MongoDB connector, you need to configure the `MONGODB_DATABASE_URI` environment variable. If you are using
Hasura DDN, this variable is automatically managed for you. You can also map it to a different environment variable as
described in the [documentation](/project-configuration/config/index.mdx#envfiles).
...
broken down into 3 folders and a root configuration file.
...
More information in the [Native Queries](/connectors/mongodb/native-operations/native-queries) documention.
...
More information in the [Native Mutations](/connectors/mongodb/native-operations/native-mutations.mdx) documentation.
...
[Native Queries](/connectors/mongodb/native-operations/native-queries) and
[Native Mutations](/connectors/mongodb/native-operations/native-mutations.mdx) will always need manual authorship
currently. We plan on improving this authorship flow in the future.
