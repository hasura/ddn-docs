...
As before, we look up the relationship object by name, generate the nested SQL using our new `fetch_relationship_rows` helper function, and this time wrap it in an `EXISTS` SQL expression.
