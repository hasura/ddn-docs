...
Let's define the where clause in `fetch_rows` as a string which is either null if there is no `where` predicate, or the
result of `visit_expression` when there is one:
