...
# NDC Connector Publication Automation Guide
...
6. Creating a new connector release and updating existing connectors
...
This guide details the automated process for publishing new connectors and updating existing versions on the [Hasura Connector Hub](https://hasura.io/connectors) and in the Hasura DDN documentation.
...
To package a connector, you will need to create a `connector-packaging.json` file for each connector version. The file contains necessary information to access the package definition. Here's the required format:
...
2. Add the following files to your connector's folder:
   ...
   - `README.md`: Documentation for your connector, including description, usage instructions, and any other relevant information.
   - `metadata.json`: Metadata file containing information about the connector.

### 5.2 Create Connector Metadata

Create a `metadata.json` file in your connector's folder. This file contains crucial information about your connector and the data present in this file is going to appear on the connector’s home page at the [connector hub](http://hasura.io/connectors). Here's a detailed structure with explanations:
...
- `is_verified`: Set this to false as only connectors developed by Hasura can be tagged as verified.Boolean indicating if the connector is verified by Hasura (initially set to `false`).

- `is_hosted_by_hasura`: Set this to false as connectors not developed by Hasura cannot be hostednot Hasura Boolean indicating if the connector can be hosted by Hasura.

- `source_code`: Information about the connector's source code.

  - `is_open_source`: Set this to true as all connectors have to be open-source. Boolean indicating if the connector is open source.
...
The connector publication automation is designed to run automatically for every commit made to a pull request targeting the `main` branch. This process ensures that your changes are continuously validated and updated in the staging environment. Here's how it works:

### 7.1 Pull Request Creation

When you create a pull request against the `main` branch with changes in the `registry` folder, it triggers the automation process.
...
- The final version of the connector (based on the last commit in the PR) is automatically published to the Production Hasura DDN.
...
- Multiple connector versions or updates to multiple connectors can be included in a single PR, and the automation will handle all changes appropriately.
...
- Issue: Automation fails due to missing environment variables
  Solution: Ping Karthikeyan Chinnakonda/Vishnu Bharathi on GitHub.

## 8. FAQs

Q: How long does the automation process take?
A: The process typically takes a few minutes, depending on the size of the connector package and the number of changes.
...
A: The process will attempt to clean up any partially uploaded files and will not update the registry database. Check the GitHub Actions log for detailed error messages.
