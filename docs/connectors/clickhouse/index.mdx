...
The Native Data Connector for ClickHouse supports all kinds of queries. In the
sections below, we'll try to give an overview of the features of the ClickHouse connector and how to configure it in a
Hasura project.
...
The [Hasura DDN CLI](/cli/overview) will help you update this configuration, which is required to see any changes to your database schema.

All commands that follow are assumed to be executed from the root directory of an already [created](/getting-started/create-a-project) Hasura DDN project.
We're using the placeholder `mydb` as source name, you may change it to something more fitting.
...
:::tip
You can either set the values in this file directly:
```yaml
...
To add models, pass the `--add-all-resources` flag to the [`ddn update connector-link`](/cli/commands/update-data-connector-link) command
...
To do this, [update the connector manifest](/connectors/clickhouse/#3-update-the-connector-manifest), and then [update the connector link](/connectors/clickhouse/#4-update-the-connector-link).
