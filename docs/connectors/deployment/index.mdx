...
You can set `SERVICE_TOKEN_SECRET` environment variable to only allow requests from authorized clients.
Set the token while deploying the connector and then create a secret on Hasura Cloud with the same value:
```bash
