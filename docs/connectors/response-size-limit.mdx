...
The maximum size limit for responses from connectors is **30 MB**. Beyond this threshold, Hasura will reject the
response to ensure optimal performance and data handling. It's important to be mindful of this limit when making data
queries to Hasura to avoid issues with data processing.

To prevent hitting the response size limit, users are encouraged to utilize the
