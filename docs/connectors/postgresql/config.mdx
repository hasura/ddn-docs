...
The configuration is a static object that lists all the database entities — such as tables — that the data connector has
to know about in order to serve queries. It never changes during the lifetime of the data connector. When your database
schema changes you will have to update the configuration accordingly.
...
Fill in default values for any fields absent from the configuration, as descriped in the
...
[Configure options](#introspectionoptions).
