...
2. Create an entry for the Native Query in the connector configuration:
   ...
   ```sh
   ddn connector plugin -c my_subgraph/connector/my_pg/connector.yaml -- \
...
metadata ([see instructions here](/docs/getting-started/build/03-connect-to-data/03-add-source-entities.mdx)). Then, you
would need to add a relationship from the generated `ProductsVectorDistance` type to the main `Products` model, like
this:
