...
The SQL structure of a Native Mutation is specified in the [Native Operation syntax](/connectors/postgresql/native-operations/syntax.mdx) page,
but require an important tweak:
...
To create a new Native Mutation, create a new SQL file inside the connector configuration directory, then use the `ddn` CLI
...
2. Create a new SQL file `my_subgraph/connector/chinook_pg/native_operations/mutations/insert_artist.sql`
...
3. Create a new entry in the ndc-postgres configuration:
   ...
   ```sh
   ddn connector plugin -c my_subgraph/connector/chinook_pg/connector.yaml -- \
     native-operation create --operation-path native_operations/mutations/insert_artist.sql --kind mutation
   ```

4. Add a source entity as described in /getting-started/build/connect-to-data/_databaseDocs/_postgreSQL/_03-add-source-entities.mdx
[Add source entities](/getting-started/build/connect-to-data/_databaseDocs/_postgreSQL/_03-add-source-entities.mdx) guide.
