...
This guide will help you set up a local Postgres database along with [Adminer](https://www.adminer.org/) to help you
manage the database using Docker Compose.
...
```dockerfile title="compose.postgres-adminer.yaml"
version: "3.9"
services:
  postgres:
...
Note that as this is a locally running database, it cannot be used to deploy your connector on Hasura DDN as the
cloud-hosted version of your data connector cannot connect to the database. You'll need to use a tool like
[ngrok](https://ngrok.com/) to provide a tunnel to access your database from the cloud. This will expose the port, most
likely `5432`, on which the database is running and allow Hasura DDN to connect to it. You can update the connector's
connection url env var in your cloud env file, e.g. `.env.cloud` with the tunnel url to deploy to Hasura DDN.
