...
Follow the [quickstart](/getting-started/quickstart.mdx) and, during the `ddn connector init -i` step, choose the
`hasura/postgres` connector. Select all the default options.
...
[this guide](/getting-started/quickstart.mdx) and providing the database connection string as the following:
