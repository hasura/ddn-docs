...
The arguments are not interpolated, but are translated to parameters of parameterized queries (arguments like
`{{id}}` and `{{name}}` are translated to `$1`, `$2`, etc.) and passed as data, and therefore can only be used in
place of scalar values, not table names, column names, or other arbitrary SQL.
...
Since you can't directly interpolate arguments (like variables) into your SQL statements in the same way
you might in other contexts, in order to use a string argument inside a pattern, such as in a `LIKE` predicate, the
argument can be concatenated with the relevant pattern parts.
