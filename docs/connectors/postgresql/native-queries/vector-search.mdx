...
For example, if we want to restrict manufacturers to being able to search vectors only for their own products,
and assuming the currently logged in manufacturer's ID is present in the session variable `x-hasura-manufacturer-id`, we would add the following to the metadata: 
