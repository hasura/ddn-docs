...
This configuration version is deprecated and should be [upgraded to the latest version](./index.mdx#upgrading-the-configuration-format-version).
...
- `sql`: SQL expression to use for the Native Query. We can interpolate values using `{{variable_name}}` syntax,
