...
The data connector will always support at least the latest two configuration versions, and tooling is provided to ease
the upgrade of configurations. A hasura cloud project will always use the head version of the PostgreSQL data connector.
We recommend doing the same for data connector instances used for local development as well.
