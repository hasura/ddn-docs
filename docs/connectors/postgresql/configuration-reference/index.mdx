...
At the time of this writing this, part of the developer experience is undergoing active development, so the exact
...
- Connect to the database with the specified `connectionUri`, and then overwrite all data in the `metadata` field
  (except for the native queries) based on the contents of the database and the values given in the
