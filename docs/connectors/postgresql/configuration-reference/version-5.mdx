...
The `queries` and `mutations` fields collect user-specified SQL queries that that may become either queryable
...
[Argument Preset](/supergraph-modeling/permissions#commandpermissions-argumentpreset) can be set in the mutation's
...
requirements. If a pre_check or post_check is not specified, the mutation will proceed without those checks by default.

## `mutationsPrefix: ""`

Configuration option mutations prefix it is an optional string which has been introduced to control the prefix used in mutation names.
