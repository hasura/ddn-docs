...
Supported since data connector version `v1.0.0.0`

:::

## Changes from the previous version

Version 5 separates the `nativeQueries` into two fields, `queries` and `mutations`, and nests them under `nativeOperations`,
as well as nested `scalarTypes` and `compositeTypes` under `types` and renames them to `scalar` and `composite` respectively.
...
The `queries` and `mutations` fields collect user-specified SQL queries that that may become either queryable collections or
...
* An update procedure is generated per table X unique constraint, and has the following form:
  ...
  ```
  Using it, we can update a single row using the uniqueness constraint by updating the relevant columns,
  and include a pre check and post check for permissions.

For each `pre_check` and `post_check` arguments of a mutation, an [Argument Preset](/supergraph-modeling/permissions#commandpermissions-argumentpreset)
should be set in the mutation's [Command Permissions](/supergraph-modeling/permissions#commandpermissions-commandpermission).
