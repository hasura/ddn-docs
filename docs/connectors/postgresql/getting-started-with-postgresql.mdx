...
you've done within PostgreSQL. The remaining work — business logic — is what requires you to maintain custom code.
...
connector and the directory in which the configuration is stored — `mypg` in this example — should match.**
...
If you're using a local PostgreSQL database — such as through [Docker](https://hub.docker.com/_/postgres) — you'll need
...
[relationships](/supergraph-modeling/relationships.mdx) between entities, allowing you to make deeply nested queries
with whatever access control rules you desire.
...
After selecting the most-recent build on your project's console, you can now execute a query using your relationship as
a field on the type in which you created it earlier. As an example, using our data model, we can now query all users'
...
instance, you add a column — such as `last_online` to a table — you can then update your `DataConnectorLink` by running
the following:
...
— optionally — providing a reason that will be passed along during schema introspection.
...
At this point, you'll have built and iterated on your supergraph locally. However, it's time to move it to Hasura DDN,
our lightning-fast, globally-deployed, and highly-reliable data delivery network that serves your API at the edge.

Deployments fall into two catgories: deploying connectors and deploying your supergrpah. Regardless of which entity
we're deploying, it's important to note that **Hasura utilizes a system of immutable
[builds](/project-configuration/builds.mdx)**. These builds work as snapshots allowing you to quickly rollout or
rollback deployments to users, worldwide. The metadata powering these builds can easily be checked into version control,
reviewed by team members, and automated using common CI/CD tools and workflows.
...
```yaml
# For example, in /ux/connector/mypg/cloud.connector.yaml
kind: Connector
version: v1
definition:
  ...
  source: hasura/postgres:<version>
  context: .
  envFile: .env.cloud
```
...
The CLI will return the console URL for your project where you can test your API from anywhere 🎉
