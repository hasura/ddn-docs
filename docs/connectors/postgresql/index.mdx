...
sections below, we'll try to give an overview of the features of the postgres connector and how to configure it in a
Hasura project.
...
The connector supports insert and delete mutations for tables. Additionally, a native query containing a DML statement
may be exposed as a mutation.

Mutations are still undergoing active development and are likely to change in the future. The configuration option
`mutationsVersion` lets you choose between currently supported variants.

Currently a single version exists: `mutationsVersion: "v1"`.
