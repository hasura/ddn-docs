...
populate the configuration, the connector supports introspecting the database via the CLI `update` command, see [updating with
...
require any DDL permissions on the database. See the configuration reference on [Native
Queries](/connectors/postgresql/native-queries/).
...
As an example (irrespective of whether this is a good database design or not) the following may be tracked with
...
Running the [ddn dev](/cli/commands/dev.mdx) command tells the CLI to watch for local development changes and update the
`ndc-postgres` configuration when changes occurs. Note that the `metadata.tables` part of the configuration will be
refreshed and overwritten when a change occurs.
...
For example, you can tweak the [Pool Settings](/connectors/postgresql/Configuration%20Reference/version-4#poolsettings),
change the [Isolation Level](/connectors/postgresql/Configuration%20Reference/version-4#isolationlevel) of transactions,
or choose [which schema not to introspect](/connectors/postgresql/Configuration%20Reference/version-4#excludedschemas).

See the [Configuration Reference](/connectors/postgresql/Configuration%20Reference) for more information.
...
More information can be found in the [Native Queries SQL Requirements](/connectors/postgresql/native-queries/index.mdx)
section.
