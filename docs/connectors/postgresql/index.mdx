...
- I ran into the error `error communicating with database`:

  If you receive such an error, make sure that the server is live and you can connect to it from your machine using `psql`.
- I'd like to tweak how the connector works or does introspection:
...
- I'm having issues getting my Native Query to run:
  ...
  or using arguments only in place of scalar values. More information can be found in the
  [Native Queries SQL Requirements](/connectors/postgresql/native-queries/index.mdx#native-queries-sql-requirements)
  section.
