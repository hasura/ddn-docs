...
description:
  "Reference documentation for the setup process for the Hasura Apache Phoenix connector, including connection URI details, and
  native queries."
keywords:
  - phoenix
  - configuration
...
The JDBC connection URL to connect to the Apache Phoenix database. This is a required field.

The value can either be a literal string, or a reference to an environment variable:

```json
{
  "jdbcUrl": "jdbc:phoenix://localhost:8090/xe?user=foo&password=bar",
  "jdbcUrl": { "variable": "PHOENIX_JDBC_URL" }
}
```
