...
1. Create a new directory structure under the connector configuration:
   ...
   ```sh
   mkdir -p my_subgraph/connector/chinook_sqlserver/native_operations/mutations/
   ```

2. Create a new SQL file `my_subgraph/connector/chinook_sqlserver/native_operations/mutations/insert_artist_and_return_id.sql` with the
   following content:

   ```sql
   INSERT INTO [dbo].[Artist] (ArtistId, Name) OUTPUT inserted.*  VALUES ({{ArtistId}}, {{Name}})
   ```

3. Create a new entry in the ndc-sqlserver configuration:
