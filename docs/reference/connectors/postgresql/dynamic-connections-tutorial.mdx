...
This tutorial demonstrates how to use dynamic connections with the PostgreSQL connector to implement read/write splitting and load balancing. We'll configure a primary database for writes and multiple read replicas for queries, with automatic routing based on operation type.

## Overview

In this setup:
- **Primary database**: Handles all mutations and mutation explanations
- **Read replicas**: Handle queries in a round-robin fashion for load distribution
- **Webhook**: Routes requests based on operation type and maintains round-robin state

## Prerequisites

This tutorial demonstrates how to use dynamic connections with the PostgreSQL connector to implement read/write splitting and load balancing. Before we begin configuring dynamic routing, you'll need to complete these setup steps:
...
For this tutorial, we'll simulate a primary-replica setup using a single PostgreSQL instance with multiple databases. In production, these would be separate database servers.
...
Set up a webhook server that will handle the dynamic routing logic. In this tutorial, we'll create a simple Node.js/Express server, but in production you would typically use your preferred hosting platform (AWS Lambda, Vercel, Railway, etc.).
...
The dynamic connection routing system uses a pre-NDC webhook to intercept requests before they reach the PostgreSQL connector, allowing us to route operations to different databases based on the operation type.
...
graph TD
    ...
    Engine -->|2. Ndc Request + Session| Webhook["Pre-NDC Hook<br />• Receive NDC request + session<br/>• Check operation type<br/>• Add connection_name parameter<br/>• Return modified request"]
    Webhook -->|3. Modified NDC Request| Engine
    Engine -->|4. Modified NDC Request| Connector[PostgreSQL Connector]
