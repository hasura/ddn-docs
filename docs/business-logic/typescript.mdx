...
If you've never used Hasura DDN, we recommend that you first go through the [getting started](/getting-started/overview.mdx).
😊
...
```bash
ddn connector-link add my_ts --configure-host http://local.hasura.dev:8083
```

Because we used the convenience flag `--configure-host` on the command, these values are already set in the
...
These values are then already referenced in `my_subgraph/metadata/my_ts/my_ts.hml`.
...
```bash
ddn connector-link update my_ts --add-all-resources
```
...
After you've generated the metadata for the function and created a new build, you can use the function in your GraphQL API.
