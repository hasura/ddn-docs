---
sidebar_position: 2
sidebar_label: Custom Business Logic via TypeScript
...
pattern known as **Command Query Separation (CQS)**.
...
In Hasura DDN, **functions** are used for read operations. They will not modify the data in the database and can only be
...
This will add a connector manifest — which contains the configuration for a connector — named `fx_connector` to the
...
With the development server running, we can iterate on our files locally with each save triggering a new build which is
...
You can add another function by writing valid TypeScript. If `ddn dev` is running, the change will automatically be
picked up the CLI, the associated metadata for the command will be generated, and you will be able to use your function
by name in GraphQL API.
