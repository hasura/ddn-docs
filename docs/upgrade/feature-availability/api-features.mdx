...
features and optimizations, making data retrieval faster and more efficient.

### Mutation

In Hasura v2, mutations enabled you to modify data in your connected databases using the auto-generated GraphQL API.

**In Hasura DDN, mutations continue to play a critical role**. The mutation capabilities have been improved, allowing
for more complex and dynamic data manipulations.

### Subscription

In Hasura v2, subscriptions provided real-time updates by automatically sending data changes to the client.
...
[GraphQL API data connector](/connectors/graphql/index.mdx). This means external GraphQL APIs are treated like any other
data source and have the full assortment of permissions and relationships at your disposal out of the box.
...
immutable builds. This allows you to iteratively develop and test your API in a safe and controlled manner, ensuring
that changes are propagated seamlessly across all environments and easily tested before deployment.
...
concept of subgraphs to allow you to create a unified API across independent teams and services, ensuring that each
subgraph can be developed and deployed independently while still being part of a larger, cohesive API.
