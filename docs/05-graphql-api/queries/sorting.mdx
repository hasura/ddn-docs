...
You can sort results based on fields from object relationships, limited to local object relationships between two
...
As stated above, `order_by` expects an object with only a single key-value pair. Should you attempt to pass multiple
key-value pairs, like in the example below where we're attempting to sort by both `lastSeen` and then `createdAt`,
you'll receive a validation error like this:
...
This happens commonly when using the explorer in GraphiQL to select `order_by` fields. Instead, you can format your
query like this wherein an array of objects is passed:
