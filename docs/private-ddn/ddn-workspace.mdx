...
You can export the metadata to your local machine by right clicking on the `Explorer` pane and selecting on the `Download` option. This will download the entire directory containing all the files in your workspace to your local machine. Workspace also has `git` pre-installed, so you can configure it and push your changes to a remote repository as well. 
