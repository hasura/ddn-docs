...
To configure the `DataConnectorLink` file you can also use the codemod `ddn codemod configure-header-forwarding` instead
of editing the `DataConnectorLink` file manually.

:::

We use [argument presets](/supergraph-modeling/data-connector-links.mdx#dataconnectorlink-dataconnectorargumentpreset)
...
[`responseHeaders` configuration property](/supergraph-modeling/data-connector-links.mdx#dataconnectorlink-responseheaders)
...
The [`outputType`](/supergraph-modeling/commands.mdx#command-commandv1) of `Command`s that represent our functions
should be the [OpenDD Scalar Type](/supergraph-modeling/types.mdx) used to represent type of the
