...
<Tabs className="language-tabs">
  ...
  <TabItem value="go" label="Go">
    ...
    // ProcedureHashPassword hashes a password string
...
```bash
# hash_password if using Python
ddn command add <connector_name> hashPassword
```
...
```graphql
mutation SimpleHashPasswordMutation {
  hashPassword(password: "super_secure_password")
}
```
