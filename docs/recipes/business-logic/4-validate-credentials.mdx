...
<Tabs className="language-tabs">
  ...
  <TabItem value="go" label="Go">
    ...
    type ComparePasswordResult struct {
    ...
    func FunctionComparePassword(ctx context.Context, state *types.State, arguments *ComparePasswordArguments) (*ComparePasswordResult, error) {
      ...
      if err != nil {
        return &ComparePasswordResult{Match: false}, nil
      }
      return &ComparePasswordResult{Match: true}, nil
    }
    ```
...
```bash
# compare_password if using Python
ddn command add <connector_name> comparePassword
```
...
```graphql
query ComparePassword {
  comparePassword(password: "user_password", hashedPassword: "stored_hashed_password")
}
```
...
```graphql
query ComparePasswordWithRelationship {
  modelWithPassword {
...
API. By leveraging lambda connectors with [relationships](/supergraph-modeling/relationships.mdx), you can add custom
business logic to your authentication flows.
