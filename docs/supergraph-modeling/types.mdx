...
| `globalIdFields` |  | `false` | The subset of fields that uniquely identify this object in the domain. Setting this property will automatically implement the GraphQL Relay Node interface for this object type and add an `id` global ID field. If setting this property, there must not be a field named `id` already present. |
| `graphql` | [`ObjectTypeGraphQLConfiguration`](#objecttypegraphqlconfiguration) | `false` | Configuration for how this object type should appear in the GraphQL schema. |
...
| Name | Type | Required | Description |
| ---- | ---- | -------- | ----------- |
| `typeName` | `string` | `false` | The name to use for the GraphQL type representation of this object type when used in an output context. |
| `inputTypeName` | `string` | `false` | The name to use for the GraphQL type representation of this object type when used in an input context. |
...
| `graphql` | [`ScalarTypeGraphQLConfiguration`](#scalartypegraphqlconfiguration) | `false` | Configuration for how this scalar type should appear in the GraphQL schema. |
