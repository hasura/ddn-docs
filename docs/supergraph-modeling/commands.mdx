...
methods to your API, or, add a custom field to an existing [model](./models.mdx) to be used in data transformation,
for example. Commands allow you to execute business logic directly from your GraphQL API and are useful in
validating, processing or enriching data or calling another API.
...
Once a model is declared it will then often be referenced by `Relationship` objects in order to compose with
each other or models, and also by `Permissions` objects to control access to the command.
