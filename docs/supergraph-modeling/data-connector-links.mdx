...
A `DataConnectorLink` belongs to a single [subgraph](/project/configuration/overview.mdxto a data source. It is used to
link the data source to the subgraph's models, commands, and relationships. The contents can be
[updated using the CLI](/cli/commands/ddn_connector-link_update.mdx). This will introspect the data source and update
the schema of the data connector.
