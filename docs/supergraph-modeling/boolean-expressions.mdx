...
Hasura provides powerful tools to control filtering and selecting data. Boolean expression types let us control which
filters are available for a model or command. They can used to configure filters on models, such as in the
[filtering](/docs/3.0/graphql-api/queries/filters/index) section, or as the types of arguments to commands or models.

### Types of boolean expression

## Scalar

This specifies how a user is able to compare a scalar field. For instance, we might want to say that we can only check
if a `String` type is equals to another, or whether it is null or not. We'd do that with the following metadata:
...
about specific data connectors - instead we specify which `BooleanExpressionType` is used to filter each field or
...
| `scalar` | [BooleanExpressionScalarOperand](#booleanexpressiontype-booleanexpressionscalaroperand) | false    | Definition of a scalar type representing a boolean expression on an OpenDD object type.  |
...
| `dataConnectorScalarType` | string                                                        | true     | Name of the scalar type according to the data connector's schema                                                                                                        |
...
| Key           | Value  | Required | Description                                  |
| ------------- | ------ | -------- | -------------------------------------------- |
| `<customKey>` | string | false    | The name of an operator in a data connector. |
...
| Key            | Value                                                 | Required | Description                                            |
| -------------- | ----------------------------------------------------- | -------- | ------------------------------------------------------ |
| `name`         | string                                                | true     | Name you want to give the operator in OpenDD / GraphQL |
...
| Key                     | Value                                                   | Required | Description |
| ----------------------- | ------------------------------------------------------- | -------- | ----------- |
| `fieldName`             | [FieldName](#booleanexpressiontype-fieldname)           | true     |             |
| `booleanExpressionType` | [CustomTypeName](#booleanexpressiontype-customtypename) | true     |             |
