...
| ---- | ---- | ----------- |
| `fieldComparison` | `object` | Field comparision predicate filters objects based on a field value. |
| `fieldIsNull` | `object` |  |
| `relationship` | `object` | Relationship predicate filters objects of a source model based on a predicate on the related model. |
| `and` | [`ModelPredicate`](#modelpredicate) |  |
| `or` | [`ModelPredicate`](#modelpredicate) |  |
| `not` |  | A predicate that can be used to restrict the objects returned when querying a model. |
