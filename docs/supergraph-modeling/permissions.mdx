...
argument called `id`, we can prepopulate that argument for the `user` role.
...
argument called `author_id`. Here we can make it so that a `user` can only
...
the user we'd like to remove, and `pre_check` lets us provide a custom boolean
expression.
