...
You'll need to ensure you've generated [ScalarTypes](/supergraph-modeling/types.mdx#scalartype-scalartype) and
[DataConnectorScalarRepresentations](/supergraph-modeling/data-connector-links.mdx#dataconnectorscalarrepresentation-dataconnectorscalarrepresentation)
...
[include an `aggregate` field in the `definition`](/supergraph-modeling/graphql-config.mdx#graphqlconfig-aggregategraphqlconfig).

Once these are included, you'll need to [create a new build](/cli/commands/ddn_supergraph_build_local.mdx) using the
CLI.
