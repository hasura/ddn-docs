...
Relationships are defined in metadata **from [object types](./types#objecttype-objecttype.mdx), to
[model](./models.mdx) or [command](./commands.mdx) types**.

This enables you to create nested queries, where you can fetch related data in a single query.

### Example 1

In an e-commerce context, you might have `Order` and `Customer` models, and you want to link them using the `order`
object type so that you can fetch a customer's details for a given order.
...
Hasura DDN also allows you to link commands together to query related data.
