...
Relationships are defined in metadata **from [object types](./types.mdx#objecttype-objecttype), to
[model](./models.mdx) or [commands](./commands.mdx)**.
...
model or command defined in the relationship.
...
type will now have a connection to the `customers` model.
