...
description:
  "Explore OrderBy Expressions in Hasura, designed for sorting data. Discover
  how to configure sorting rules for various data types and expose them through
  your API."
keywords:
...
Hasura provides powerful tools to control sorting and ordering data. OrderBy Expression types let you define customizable 
sorting rules for a [model](/supergraph-modeling/models.mdx). They can be used to configure sorting on models, such as in the [filtering](graphql-api/queries/filters/index.mdx)
section, or as the types of arguments to commands or models.
...
| Type              | Description                                               |
| ----------------- | --------------------------------------------------------- |
| [Scalar](#scalar) | This specifies how a user able to order a scalar field. |
| [Object](#object) | This specifies how fields of a type can be ordered.      |

Regardless of the type, orderByExpressions are used to define how a user is able to sort data and are defined in your
metadata.
...
This specifies how a user is able to order a scalar field. For instance, you might want to say that a user can only
sort a `String` type in ascending order instead of both ascending and descending order. You can do that with the following
metadata:
...
`OrderByExpression` is used to order each field or relationship, and then defer the mappings of individual 
scalar types to those `OrderByExpression`s.
