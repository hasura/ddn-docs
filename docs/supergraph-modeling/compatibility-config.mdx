...
A build error is now raised in two scenarios:

1. Duplicate names within a subgraph for the following expression definitions:
   - `BooleanExpressionType`
   - `OrderByExpression`

2. Type name conflicts across the following type definitions:
   - `ScalarType`
   - `ObjectType`
   - `BooleanExpressionType`
