...
**Description:** When enabled, raise an error when a command uses a GraphQL
name that is already used by another field. Before this, the command would be
silently dropped from the GraphQL schema.
